...
==========================================
Purpose: Select the best image from multiple source articles in a cluster
Model: Rule-based multi-factor scoring (no AI needed)
Input: Cluster with N source articles (with image URLs)
Output: Best image URL + metadata
Key Feature: Scores by source reputation, dimensions, aspect ratio, format
...
class ImageSelector:
    ...
    def select_best_image(self, sources: List[Dict], article_title: str = "") -> Optional[Dict]:
        ...
        Args:
            ...
            article_title: Article title (for future relevance check)
        ...
        for source in sources:
            ...
            candidates.append({
                ...
                'article_score': source.get('score', 50),  # Gemini score from Step 1
...
def select_best_image_for_cluster(cluster_sources: List[Dict], cluster_title: str = "") -> Optional[Dict]:
    """
    Main function to select best image for a cluster
    
    Args:
        cluster_sources: List of source articles in the cluster
        cluster_title: Main cluster title (optional)
    
    Returns:
        Dict with {url, source_name, quality_score} or None
    """
    selector = ImageSelector(debug=True)
    return selector.select_best_image(cluster_sources, cluster_title)
...
if __name__ == "__main__":
    ...
    result = select_best_image_for_cluster(test_sources, "AI Chip Breakthrough")
