...
// ============================================================================
// 20-PERSONA FULL EXPERIENCE TEST V3 — NEW ALGORITHM (16 improvements)
// - ALL 20 personas start FRESH (zero history) to test new system from scratch
...
// - HTTP timeouts to prevent API hangs
// ============================================================================
...
const HTTP_TIMEOUT_MS = 15000; // 15s timeout to prevent API hangs
...
async function fetchExplorePage(userId) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    try {
      ...
      if (attempt === 0 && e.message.includes('timeout')) {
        console.log(`    [Explore API timeout, retrying...]`);
        await sleep(2000);
        continue;
      }
      throw e;
...
async function setupPersona(persona) {
  ...
  // V3: ALL personas start fresh regardless of keepHistory flag
  {
...
function generateFeedback(persona, feedResult, exploreBefore, exploreAfter) {
  ...
  // ── DETAILED COMPLAINTS: analyze exactly what was bad ──
  const complaints = [];
  const detailedComplaints = [];

  // Find categories that persona doesn't follow but kept seeing
  ...
  const skippedTitles = [];
  const worstArticles = []; // articles with lowest score that were irrelevant
  ...
  // SPECIFIC detailed complaints with data
  ...
  if (topUnwanted.length > 0) {
    for (const [cat, cnt] of topUnwanted.slice(0, 3)) {
      detailedComplaints.push(`I don't follow ${cat} but was shown ${cnt} article${cnt > 1 ? 's' : ''} from it — waste of my time`);
    }
  }
  if (skippedTitles.length >= 5) {
    detailedComplaints.push(`I skipped ${skippedTitles.length} articles that had NOTHING to do with my interests (${persona.subtopics.slice(0, 2).join(', ')})`);
  }
  if (worstArticles.length > 0) {
    const worst = worstArticles[0];
    detailedComplaints.push(`Worst article for me: "${worst.title}" (${worst.category}) — score ${worst.score.toFixed(2)}, spent only ${worst.dwell.toFixed(1)}s on it`);
  }
  ...
  if (dominantCat && dominantCat[1] >= total * 0.4) {
    detailedComplaints.push(`${dominantCat[1]} out of ${total} articles were ${dominantCat[0]} — too much of one category, felt repetitive`);
  }

  // Low dwell complaint
  if (avgDwell < 3.5) {
    detailedComplaints.push(`My average dwell was only ${avgDwell.toFixed(1)}s per article — nothing grabbed my attention long enough`);
  }

  // Early exit complaint
  if (total < 10 && (exit === 'bored' || exit === 'frustrated')) {
    detailedComplaints.push(`I only made it through ${total} articles before giving up — that's a really bad first impression`);
  }

  // Feed didn't improve over time
  if (feedResult.scrollWindows.length >= 2) {
    const firstW = feedResult.scrollWindows[0];
    const lastW = feedResult.scrollWindows[feedResult.scrollWindows.length - 1];
    if (lastW.engRate <= firstW.engRate && lastW.endMood < firstW.endMood) {
      detailedComplaints.push(`Feed got WORSE as I scrolled — engagement went from ${(firstW.engRate * 100).toFixed(0)}% to ${(lastW.engRate * 100).toFixed(0)}%, my mood dropped from ${firstW.endMood} to ${lastW.endMood}`);
    }
  }

  // Missing content complaint — subtopics that had zero matches
  ...
  if (missingSTs.length > 0) {
    detailedComplaints.push(`I follow ${missingSTs.join(', ')} but saw ZERO articles matching ${missingSTs.length > 1 ? 'these topics' : 'this topic'}`);
  }

  // Specific article examples the persona liked
  ...
  // Specific quote based on persona character — MORE DETAILED
  let personalQuote;
  if (engRate >= 0.5 && mood >= 50) {
    const bestST = persona.subtopics[0];
    personalQuote = `As a ${persona.bio.split('.')[0].toLowerCase()}, I found plenty of ${bestST} content. Spent ${totalDwell.toFixed(0)}s total, engaged with ${engaged} articles. I'd come back daily.`;
  } else if (relRate >= 0.4 && engRate < 0.3) {
    personalQuote = `The articles touch on ${persona.subtopics[0]} but they're too surface-level. I read ${total} articles, only deeply engaged with ${engaged}. Average dwell was ${avgDwell.toFixed(1)}s — I need deeper, more niche content.`;
  } else if (exit === 'frustrated') {
    personalQuote = `I'm into ${persona.subtopics.slice(0, 2).join(' and ')} but ${skippedTitles.length} of ${total} articles were completely irrelevant. Left after ${totalDwell.toFixed(0)}s feeling frustrated. ${missingSTs.length > 0 ? `Never saw a single ${missingSTs[0]} article.` : ''}`;
  } else if (exit === 'bored') {
    personalQuote = `Started okay but got repetitive after article ${Math.min(10, total)}. ${dominantCat ? `Too much ${dominantCat[0]} (${dominantCat[1]}/${total} articles).` : ''} My mood dropped from 55 to ${mood}. Would need more variety to keep me.`;
  } else {
    personalQuote = `It's okay — ${engaged}/${total} articles were worth reading, but ${total - relevant} didn't match my interests (${persona.subtopics.slice(0, 2).join(', ')}). ${avgDwell.toFixed(1)}s avg dwell tells the story.`;
  }
  ...
  if (missingSTs.length > 0) featureRequests.push(`Need more ${missingSTs[0]} content — I saw zero articles for this topic`);
...
async function main() {
  const T = '='.repeat(110);
  console.log(T);
  console.log('  20-PERSONA FULL EXPERIENCE TEST V3 — NEW ALGORITHM');
  console.log('  ALL 20 personas start FRESH (zero history) — testing new unified scoring');
  ...
  for (let i = 0; i < PERSONAS.length; i++) {
    ...
    const historyLabel = 'FRESH'; // V3: all personas start fresh
    ...
    // Setup
    console.log(`    Setting up (wiping history — fresh start)...`);
    ...
    console.log(`\n    DETAILED ARTICLE-BY-ARTICLE VIEW:`);
    console.log(`    ${'#'.padEnd(5)} ${'Action'.padEnd(12)} ${'Dwell'.padStart(7)} ${'Mood'.padStart(6)} ${'Score'.padStart(7)} ${'Save'.padStart(6)} ${'Category'.padEnd(16)} ${'Matched Subtopics'.padEnd(30)} Title`);
    console.log(`    ${'─'.repeat(140)}`);
    for (const ix of ints) {
      const dwellStr = ix.dwell.toFixed(1) + 's';
      const saveStr = ix.save ? 'YES' : '';
      const matchStr = ix.matchedSubtopics.length > 0 ? ix.matchedSubtopics.join(', ').substring(0, 28) : '(none)';
      const actionIcon = ix.action === 'DEEP_READ' ? 'DEEP_READ' : ix.action === 'ENGAGE' ? 'ENGAGE' : ix.action === 'GLANCE' ? 'GLANCE' : ix.action === 'SCAN' ? 'SCAN' : 'SKIP';
      console.log(`    ${String(ix.num).padEnd(5)} ${actionIcon.padEnd(12)} ${dwellStr.padStart(7)} ${String(ix.mood).padStart(6)} ${ix.score.toFixed(2).padStart(7)} ${saveStr.padStart(6)} ${(ix.category || '-').padEnd(16)} ${matchStr.padEnd(30)} ${(ix.title || '').substring(0, 55)}`);
    }

    // Dwell time summary by action type
    const dwellByAction = {};
    for (const ix of ints) {
      if (!dwellByAction[ix.action]) dwellByAction[ix.action] = [];
      dwellByAction[ix.action].push(ix.dwell);
    }
    console.log(`\n    DWELL TIME SUMMARY BY ACTION:`);
    for (const [action, dwells] of Object.entries(dwellByAction).sort((a, b) => b[1].length - a[1].length)) {
      ...
      const min = Math.min(...dwells);
      const max = Math.max(...dwells);
      console.log(`    ${action.padEnd(12)} Count: ${String(dwells.length).padStart(3)} | Avg: ${avg.toFixed(1).padStart(5)}s | Min: ${min.toFixed(1).padStart(5)}s | Max: ${max.toFixed(1).padStart(5)}s | Total: ${dwells.reduce((s, d) => s + d, 0).toFixed(0).padStart(5)}s`);
    }
    const totalDwell = ints.reduce((s, ix) => s + ix.dwell, 0);
    console.log(`    ${'TOTAL'.padEnd(12)} Count: ${String(ints.length).padStart(3)} | Avg: ${(totalDwell / Math.max(ints.length, 1)).toFixed(1).padStart(5)}s | Total session: ${totalDwell.toFixed(0)}s (${(totalDwell / 60).toFixed(1)} min)`);

    // Category breakdown for this persona
    const catStats = {};
    for (const ix of ints) {
      const c = ix.category || 'Unknown';
    ...
    console.log(`\n    CATEGORY BREAKDOWN FOR ${persona.name.toUpperCase()}:`);
    console.log(`    ${'Category'.padEnd(18)} ${'Shown'.padStart(6)} ${'Eng'.padStart(5)} ${'Skip'.padStart(6)} ${'Rel'.padStart(5)} ${'AvgDwell'.padStart(9)} ${'Eng%'.padStart(7)} ${'Wanted?'.padStart(9)}`);
    ...
    for (const [cat, st] of Object.entries(catStats).sort((a, b) => b[1].total - a[1].total)) {
      const wanted = personaCats.has(cat) ? 'YES' : 'no';
      console.log(`    ${cat.padEnd(18)} ${String(st.total).padStart(6)} ${String(st.engaged).padStart(5)} ${String(st.skipped).padStart(6)} ${String(st.relevant).padStart(5)} ${(st.dwellSum / st.total).toFixed(1).padStart(8)}s ${((st.engaged / st.total * 100).toFixed(0) + '%').padStart(7)} ${wanted.padStart(9)}`);
    }

    // Articles this persona HATED (skipped with low score)
    const hatedArticles = ints.filter(ix => ix.action === 'SKIP' && ix.matchedSubtopics.length === 0);
    const unwantedCategories = {};
    for (const ix of hatedArticles) {
      const c = ix.category || 'Unknown';
      if (!personaCats.has(c)) {
        unwantedCategories[c] = (unwantedCategories[c] || 0) + 1;
      }
    }
    if (Object.keys(unwantedCategories).length > 0) {
      console.log(`\n    UNWANTED CONTENT — Categories ${persona.name} doesn't follow but kept seeing:`);
      for (const [cat, cnt] of Object.entries(unwantedCategories).sort((a, b) => b - a)) {
        console.log(`    - ${cat}: ${cnt} irrelevant article${cnt > 1 ? 's' : ''} shown`);
      }
    }
    ...
    console.log(`    Total time in feed: ${feedback.totalDwell.toFixed(0)}s (${(feedback.totalDwell / 60).toFixed(1)} min) | Avg dwell: ${feedback.avgDwell.toFixed(1)}s/article`);
    ...
    if (feedback.detailedComplaints.length > 0) {
      console.log(`    DETAILED COMPLAINTS (what ${persona.name} didn't like):`);
      for (const dc of feedback.detailedComplaints) {
        console.log(`      - ${dc}`);
      }
    }
    if (feedback.missingSTs.length > 0) {
      console.log(`    MISSING CONTENT: ${feedback.missingSTs.join(', ')} — followed but never shown`);
    }
    if (feedback.likedExamples.length > 0) {
      console.log(`    ARTICLES THEY LIKED: ${feedback.likedExamples.join(' | ')}`);
    }
    if (feedback.worstArticle) {
      console.log(`    WORST ARTICLE: "${feedback.worstArticle.title}" (${feedback.worstArticle.category}) — score: ${feedback.worstArticle.score.toFixed(2)}, dwell: ${feedback.worstArticle.dwell.toFixed(1)}s`);
    }
    ...
    for (const fr of feedback.featureRequests) {
  ...
  console.log(`\n\n${'='.repeat(130)}`);
  console.log('  GRAND REPORT — 20-PERSONA FULL EXPERIENCE TEST V3 (NEW ALGORITHM)');
  ...
  console.log('\n  TABLE 4: PERFORMANCE BY INTEREST CATEGORY');
  ...
  console.log(`  ${'Category'.padEnd(30)} ${'Personas'.padStart(10)} ${'Avg Eng%'.padStart(10)} ${'Avg Mood'.padStart(10)} ${'Avg Rating'.padStart(12)} ${'Explore After'.padStart(14)}`);
  ...
  for (const [cat, group] of Object.entries(categoryGroups).sort((a, b) => b[1].length - a[1].length)) {
    ...
    console.log(`  ${cat.padEnd(30)} ${String(group.length).padStart(10)} ${((avgEng*100).toFixed(1)+'%').padStart(10)} ${avgMood.toFixed(0).padStart(10)} ${(avgRating.toFixed(1)+'/10').padStart(12)} ${(avgExp.toFixed(1)+'%').padStart(14)}`);
  }

  // Also show overall stats for the V2 comparison
  ...
  const freshStats = groupStats(allResults);
  const retStats = freshStats; // same group for V3 (all fresh)

  // ── TABLE 5: DWELL TIME & SESSION DEPTH ──
  console.log('\n  TABLE 5: DWELL TIME & SESSION DEPTH');
  console.log(`  ${'Name'.padEnd(12)} ${'Articles'.padStart(9)} ${'TotalDwell'.padStart(11)} ${'SessionMin'.padStart(11)} ${'AvgDwell'.padStart(9)} ${'DeepReads'.padStart(10)} ${'DeepAvg'.padStart(9)} ${'SkipAvg'.padStart(9)} ${'Missing'.padStart(9)}`);
  console.log(`  ${'─'.repeat(100)}`);

  for (const r of allResults) {
    const ints = r.feedResult.interactions;
    const deepReads = ints.filter(i => i.action === 'DEEP_READ');
    const skips = ints.filter(i => i.action === 'SKIP');
    const deepAvg = deepReads.length > 0 ? deepReads.reduce((s, i) => s + i.dwell, 0) / deepReads.length : 0;
    const skipAvg = skips.length > 0 ? skips.reduce((s, i) => s + i.dwell, 0) / skips.length : 0;
    console.log(`  ${r.persona.name.padEnd(12)} ${String(ints.length).padStart(9)} ${(r.feedback.totalDwell.toFixed(0) + 's').padStart(11)} ${(r.feedback.totalDwell / 60).toFixed(1).padStart(10)}m ${(r.feedback.avgDwell.toFixed(1) + 's').padStart(9)} ${String(deepReads.length).padStart(10)} ${(deepAvg.toFixed(1) + 's').padStart(9)} ${(skipAvg.toFixed(1) + 's').padStart(9)} ${String(r.feedback.missingSTs.length).padStart(9)}`);
  }

  // ── TABLE 6: User Feedback Summary ──
  console.log('\n  TABLE 6: PERSONA FEEDBACK SUMMARY');
  ...
  // ── TABLE 7: DETAILED COMPLAINTS — what each persona didn't like ──
  console.log('\n  TABLE 7: WHAT EACH PERSONA DIDN\'T LIKE (DETAILED)');
  console.log(`  ${'─'.repeat(120)}`);
  for (const r of allResults) {
    const f = r.feedback;
    console.log(`  ${r.persona.name.toUpperCase()} (${f.overallRating}/10, ${f.feedSatisfaction}):`);
  ...
  // ── TABLE 8: Feature Requests ──
  console.log('\n  TABLE 8: FEATURE REQUESTS');
  console.log(`  ${'Name'.padEnd(12)} Feature Requests`);
  console.log(`  ${'─'.repeat(100)}`);
  for (const r of allResults) {
    const f = r.feedback;
    console.log(`  ${r.persona.name.padEnd(12)} ${f.featureRequests.join(' | ')}`);
  }

  // ── TABLE 9: Persona Quotes ──
  console.log('\n  TABLE 9: WHAT USERS SAY');
  console.log(`  ${'─'.repeat(120)}`);
  for (const r of allResults) {
    console.log(`  ${r.persona.name.padEnd(10)} (${r.feedback.overallRating}/10): "${r.feedback.personalQuote}"`);
  }

  // ── TABLE 10: Articles They Liked ──
  console.log('\n  TABLE 10: ARTICLES EACH PERSONA LIKED MOST');
  console.log(`  ${'─'.repeat(120)}`);
  for (const r of allResults) {
    if (r.feedback.likedExamples.length > 0) {
      console.log(`  ${r.persona.name.padEnd(10)} ${r.feedback.likedExamples.join(' | ')}`);
    } else {
      console.log(`  ${r.persona.name.padEnd(10)} (no deep reads or saves)`);
    }
  }
  ...
  const personasWithComplaints = allResults.filter(r => r.feedback.detailedComplaints.length > 0).length;
  ...
  console.log(`  Would recommend:             ${wouldRecommend}/${allResults.length} (${((wouldRecommend / allResults.length) * 100).toFixed(0)}%)`);
  console.log(`  Avg articles before exit:    ${freshStats.avgArticles.toFixed(1)}`);
  ...
  console.log(`  Personas with complaints:    ${personasWithComplaints}/${allResults.length}`);
  console.log(`  Explore BEFORE relevance:    ${freshStats.avgExpBefore.toFixed(1)}%`);
  console.log(`  Explore AFTER relevance:     ${freshStats.avgExpAfter.toFixed(1)}%`);
