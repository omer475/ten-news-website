    ...
    var body: some View {
        ...
        .sheet(isPresented: $showFlashBrief) {
            FlashBriefSheet(
                ...
                onTopicTap: { topic in
                    // Phase 3.x chip-tap signal: fire entity_chip_tap analytics
                    // (server writes the entity signal at weight ~0.8, between
                    // save and share). Then dismiss the brief and push the topic
                    // feed so the user can dive into that entity's coverage.
                    Task {
