...
private struct WelcomeScene: View {
    ...
    private func fetchArticles() async {
        ...
        do {
            ...
            // Take the top 3 articles from each topic (have images), shuffle for variety
            var pool: [ExploreTopicArticle] = []
            var seenIDs = Set<String>()
            for topic in response.topics {
                let withImages = topic.articles.filter { $0.imageUrl != nil }
                for article in withImages.prefix(3) {
    ...
    private func articleCard(at index: Int, depth: Int) -> some View {
        ...
        let scale: CGFloat = [1.0, 0.94, 0.88][depth]
        let yOffset: CGFloat = [0, 12, 24][depth]
        let opacity: Double = [1.0, 0.65, 0.35][depth]
        ...
        // Soft ambient glow — symmetric, diffuse, no offset (not a drop shadow)
        .shadow(color: color.opacity(0.10 * opacity), radius: 44)
        .scaleEffect(scale)
        .offset(y: yOffset)
        .opacity(opacity)
        .blur(radius: depth == 2 ? 1 : 0)
        .zIndex(Double(-depth))
        .id(depth == 0 ? "front-\(cardIndex)" : "depth-\(depth)")
        .transition(
            depth == 0
    ...
    private var tagline: some View {
        Text("the feed that gets what you're into.")
            .font(.system(size: 14, weight: .medium, design: .rounded))
            .foregroundStyle(.white.opacity(0.5))
            .tracking(0.2)
            .opacity(taglineVisible ? 1 : 0)
            .blur(radius: taglineVisible ? 0 : 6)
    }
