...
## Purpose
Score news articles from RSS feeds (0-1000) to filter for "must-know" news only. Articles scoring 700+ are approved and passed to Step 2 (Jina) for full article fetching. Uses holistic judgment based on significance, impact, and quality.

---

## Workflow Position

**This is Step 1 in the complete news processing pipeline:**

```
RSS Feed (1000 articles with title + source + description + URL)
    ↓
STEP 1: Gemini scores all articles (THIS DOCUMENT)
    → Result: ~150 approved articles (score ≥700)
    ↓
STEP 2: Jina fetches full text for approved articles only
    ↓
STEP 3: Claude writes title + summary
    ↓
STEP 4: Perplexity searches for context
    ↓
STEP 5: Claude formats timeline + details
```

**Key point:** Gemini filters using only RSS data (title + source + description). This is intentional - we don't fetch full articles until after filtering to save resources.
  ...
  "contents": [
    {
      ...
      "parts": [
        {
          "text": "[USER PROMPT WITH ARTICLES - SEE BELOW]"
        }
      ]
    }
  ],
...
```
You are a news curator AI for a global news application. Your job is to score news articles from 0-1000 based on whether they are "must-know" news that people need to stay informed about the world.
...
```
Score these news articles based on must-know criteria. Return JSON array only.
...
[
  {
    ...
    "source": "Reuters",
    "text": "The ECB announced Thursday it is raising rates by 0.25 percentage points...",
...
**Note:** The `url` field is included in input so you can pass it to Step 2, but Gemini doesn't score based on URL.
