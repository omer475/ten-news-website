...
async function testLoadHappyPath() {
  // User has 9 primary-62 hits 5 min ago + 4 secondary-494 hits 8 min ago.
  const now = Date.now()
  const rows = [
    { axis: 'primary',   value: '62',  count: 9, last_updated_at: new Date(now - 5*60_000).toISOString() },
    { axis: 'primary',   value: '19',  count: 2, last_updated_at: new Date(now - 3*60_000).toISOString() },
  ...
  // Article in primary 62 from SCMP → multiplier should be well below 1.0.
  const m62 = x.multiplierFor({ vq_primary: 62, source: 'SCMP' })
  assert(m62 < 0.7,
    'happy: primary-62 + source SCMP discounted < 0.7',
    `got ${m62.toFixed(3)}`)

  // Article in primary 19 (only 2 hits) from a fresh source → milder discount.
  const m19 = x.multiplierFor({ vq_primary: 19, source: 'TechCrunch' })
  assert(m19 > 0.7 && m19 < 1.0,
    'happy: primary-19 + fresh source: mild discount',
    `got ${m19.toFixed(3)}`)
