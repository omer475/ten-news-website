...
// ---------------------------------------------------------------------------
// PR3 (2026-05-18) — tighter freshness fallbacks for news-tagged content.
// Only fires when shelf_life_days is missing; per-row value still wins.
// ---------------------------------------------------------------------------

function testRecency_breakingFreshnessFallback() {
  // No shelf_life_days, freshness_category='breaking' → fallback 1d.
  // Half-life = 12h, so 12h-old article ≈ 0.5×.
  const now = new Date('2026-05-18T13:00:00Z').getTime()
  const a = {
    created_at: new Date(now - 12 * 3600 * 1000).toISOString(),
    freshness_category: 'breaking',
    // shelf_life_days intentionally omitted
  }
  const w = recencyWeightForArticle(a, now)
  assert(Math.abs(w - 0.5) < 0.02,
    'recency: breaking-fallback @ 12h ≈ 0.5 (1d half-life)',
    `got ${w.toFixed(3)}`)
}

function testRecency_newsFreshnessFallback() {
  // No shelf_life_days, freshness_category='news' → fallback 2d.
  // Half-life = 24h, so 24h-old article ≈ 0.5×.
  const now = new Date('2026-05-18T13:00:00Z').getTime()
  const a = {
    created_at: new Date(now - 24 * 3600 * 1000).toISOString(),
    freshness_category: 'news',
  }
  const w = recencyWeightForArticle(a, now)
  assert(Math.abs(w - 0.5) < 0.02,
    'recency: news-fallback @ 24h ≈ 0.5 (2d half-life)',
    `got ${w.toFixed(3)}`)
}

function testRecency_explicitShelfLifeWinsOverFreshnessCategory() {
  // Per-row shelf_life_days=5 + freshness_category='breaking' → uses 5d,
  // NOT the 'breaking' fallback of 1d. AI-pipeline-set value is authoritative.
  // Half-life = 60h, so 60h-old article ≈ 0.5×.
  const now = new Date('2026-05-18T13:00:00Z').getTime()
  const a = {
    created_at: new Date(now - 60 * 3600 * 1000).toISOString(),
    shelf_life_days: 5,
    freshness_category: 'breaking',
  }
  const w = recencyWeightForArticle(a, now)
  assert(Math.abs(w - 0.5) < 0.02,
    'recency: explicit shelf_life_days overrides freshness_category fallback',
    `got ${w.toFixed(3)} expected ≈ 0.5 with 5d/60h half-life`)
}

function testRecency_unknownFreshnessCategoryFallsThroughToDefault() {
  // freshness_category not in {breaking, news} → falls through to
  // DEFAULT_SHELF_LIFE_DAYS=3 → half-life 36h.
  const now = new Date('2026-05-18T13:00:00Z').getTime()
  const a = {
    created_at: new Date(now - 36 * 3600 * 1000).toISOString(),
    freshness_category: 'feature',  // not in fallback map
  }
  const w = recencyWeightForArticle(a, now)
  assert(Math.abs(w - 0.5) < 0.02,
    'recency: unknown freshness_category → default 3d shelf life',
    `got ${w.toFixed(3)} expected ≈ 0.5 with default 3d/36h half-life`)
}
...
testRecency_breakingFreshnessFallback()
testRecency_newsFreshnessFallback()
testRecency_explicitShelfLifeWinsOverFreshnessCategory()
testRecency_unknownFreshnessCategoryFallsThroughToDefault()
...
function testDiversityDiscountStoryFloorAt0_15() {
  // 2026-05-18 — story-axis floor restored to 0.15 (was 0.05 in Wave 1).
  // Build 8 items in the same story cluster but with DIFFERENT vq_primary,
  // vq_secondary, source, author so only the story axis fires. With floor
  // 0.15 and decay 0.5, the 8th item's story multiplier is
  // (1-0.15)·0.5^7 + 0.15 = 0.85/128 + 0.15 ≈ 0.157, raw 100 → ~15.7.
  const slate = []
  for (let i = 0; i < 8; i++) {
    slate.push({
      id: i + 1,
  ...
  assert(minScore > 14.0 && minScore < 17.0,
    'applyDiversityDiscount: 8th same-cluster item lands at floor 0.15× raw',
    `minScore=${minScore.toFixed(3)} (expected ≈ 15.7 with floor 0.15)`)
}

function testDiversityDiscountSecondaryAxis() {
  // 2026-05-18 — new 'secondary' axis (Trinity h2 sub-cluster). Build 4
  // items all in the SAME vq_secondary, with different vq_primary,
  // story cluster, source, author so only the secondary axis fires.
  // Floor 0.25, decay 0.5: 4th item's secondary multiplier is
  // (1-0.25)·0.5^3 + 0.25 = 0.75/8 + 0.25 = 0.34375, raw 100 → ~34.4.
  const slate = [
    { id: 1, vq_primary: 39, vq_secondary: 500, _storyClusterId: 'c1', source: 'S1', author_id: 'a1', _score: 100 },
    { id: 2, vq_primary: 40, vq_secondary: 500, _storyClusterId: 'c2', source: 'S2', author_id: 'a2', _score: 100 },
    { id: 3, vq_primary: 41, vq_secondary: 500, _storyClusterId: 'c3', source: 'S3', author_id: 'a3', _score: 100 },
    { id: 4, vq_primary: 42, vq_secondary: 500, _storyClusterId: 'c4', source: 'S4', author_id: 'a4', _score: 100 },
  ]
  const out = applyDiversityDiscount(slate)
  // Scores should monotonically decrease as secondary-axis count climbs.
  for (let i = 1; i < out.length; i++) {
    assert(out[i]._score < out[i - 1]._score,
      `secondary axis: item at position ${i} scores below position ${i - 1}`,
      `${out[i]._score.toFixed(3)} vs ${out[i - 1]._score.toFixed(3)}`)
  }
  const lastScore = out[out.length - 1]._score
  assert(lastScore > 33 && lastScore < 36,
    'secondary axis: 4th same-h2-cluster item lands at ≈ 0.344× raw',
    `last=${lastScore.toFixed(3)}`)
}

function testDiversityDiscountSecondaryAxisIgnoresNull() {
  // Items without vq_secondary (legacy fixtures) should pass through unscathed
  // on the secondary axis — same as how the primary/source/author axes skip
  // when the value is null.
  const slate = [
    { id: 1, vq_primary: 39, _storyClusterId: 'c1', source: 'S1', author_id: 'a1', _score: 100 },
    { id: 2, vq_primary: 40, _storyClusterId: 'c2', source: 'S2', author_id: 'a2', _score: 100 },
  ]
  const out = applyDiversityDiscount(slate)
  for (const it of out) {
    assert(it._score === 100,
      'secondary axis: null vq_secondary skipped, score unchanged',
      `_score=${it._score}`)
  }
}
...
function testListwiseCrossSlateSeedsStoryAxis() {
  ...
  // 2026-05-18 — story floor restored to 0.15. With 5 prior impressions:
  //   mult = (1-0.15)·0.5^5 + 0.15 = 0.85/32 + 0.15 = 0.0266 + 0.15 ≈ 0.177
  //   raw 100 → ≈ 17.7. Pre-PR2 floor was 0.05 → ≈ 0.08× → 8.0.
  assert(c1Item._score < 20 && c1Item._score > 14,
    'cross-slate seed: c1 with 5 prior impressions drops to ≈ 0.177× raw (floor 0.15)',
...
testDiversityDiscountStoryFloorAt0_15()
testDiversityDiscountSecondaryAxis()
testDiversityDiscountSecondaryAxisIgnoresNull()
