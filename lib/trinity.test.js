...
function testDiversityDiscountSamePrimary() {
  // 5 items all from primary=39, scored 100/90/80/70/60.
  // Expected after discount on primary axis (default):
  //   #1: 100 * 1.000 = 100.00
  //   #2:  90 * 0.625 =  56.25
  //   #3:  80 * 0.4375 = 35.00
  //   #4:  70 * 0.34375 = 24.06
  ...
  // Top-of-slate is the original highest-score item (no discount applied to first).
  ...
  // 2nd in slate: most-discounted-but-still-highest. Could be the original #2 (90*0.625=56.25).
  assert(Math.abs(out[1]._score - 56.25) < 0.5,
    '2nd item discounted by 0.625× (1 prior on primary)',
...
function testStoryClusterConstantLocked() {
  assert(Math.abs(STORY_CLUSTER_THRESHOLD - 0.55) < 1e-9,
    'STORY_CLUSTER_THRESHOLD locked at 0.55 (Google News BERT pattern)',
