...
function testDiversityDiscountStoryFloorLowered() {
  // 2026-05-17 — story-axis floor lowered 0.20 → 0.05. Build 8 items in the
  // same story cluster but all with DIFFERENT vq_primary and DIFFERENT source
  // so the primary and source axes don't fire. With floor 0.05 and
  // decay 0.5, the 8th item's story multiplier is
  ...
  applyDiversityDiscount(slate)
  // Find the item that was processed last (lowest adjusted _score).
  const minScore = Math.min(...slate.map(it => it._score))
  // 100 * 0.0574 = 5.74
  assert(minScore < 8.0,
