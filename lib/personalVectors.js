...
// 2026-05-18 — buffer cap raised 1000 → 5000.
//
// Production audit (test user 5082a1df) revealed a feedback loop: with the
// 1000-event cap, ~6 days of recent algorithm-bug-era serving had drowned
// out the user's true long-term interests. Primary 39 (Tech, h1=1929, user's
// #1 interest) had 155 deep-read events in the last 60 days but only 15 in
// the buffer — recent serving had filled the buffer with primary 62 China
// content (48 rows) instead. Vector for primary 39 was built from 15 events,
// so ANN retrieval for fresh Tech articles came back empty, slate fell back
// to long-tail, loop tightened.
//
// Peer-platform research (primary sources):
//   * Pinterest PinnerSage (arxiv:2007.03634 §3.1) — 90-day window of action
//     pins, no count cap; recency decay λ=0.01/day at the importance-score
//     layer (~69-day half-life).
//   * Pinterest PinnerFormer (arxiv:2205.04507 §4.2.7) — single global
//     sequence, M=256 most recent.
//   * TikTok Trinity (arxiv:2402.02842 §4.1) — global behavior sequence,
//     length=2500.
//   * YouTube DNN (Covington 2016 §3.1, §6) — 50 most recent watches, mean-
//     pooled. Same shape, different scale.
//   * Instagram DV365 (arxiv:2506.00450) — 40K-70K global timeline.
//
// All peers use a global recency cap (Option A). Per-cluster top-K (the
// approach in the abandoned PR6 v1 / #194) appears only in candidate-
// conditioned long-sequence ranking (Alibaba SIM, Kuaishou TWIN-V2), NOT
// in user-vector construction.
//
// 5000 was chosen to match Trinity's 2500 with headroom. For heavy users
// (~74 deep-reads/day for the audited account), 5000 covers ~67 days —
// enough to retain all of primary 39's 155 deep-reads + survive 1-2 weeks
// of bad serving without losing long-term taste.
//
// Env override: TRINITY_ENGAGEMENT_BUFFER_LIMIT for live tuning without
// redeploy. Floored at 200 to prevent accidental disable.
//
// 2026-05-19 (PR8) — tuned 5000 → 2500. PR #195 fixed the 1000-cap supply bug
// by going to 5000, but 5000 doubled the network payload (~7.5 MB/request)
// causing slow loading. 2500 matches TikTok Trinity's published value exactly
// (arxiv:2402.02842 §4: "long-term behavior sequence length=2500"). Heavy
// user audit confirms 2500 still captures primary 39's 155 deep-reads.
export const ENGAGEMENT_BUFFER_LIMIT = Math.max(
  200,
  Number(process.env.TRINITY_ENGAGEMENT_BUFFER_LIMIT) || 2500
)
