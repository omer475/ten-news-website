...
function testEngagementZDirectionInvariant() {
  ...
  assert(engaged['trinity-personal'] >= bored['trinity-personal'],
    'engagementZ direction (heavy): engaged user gets >= personal (exploit-heavy)',
    `engaged.personal=${engaged['trinity-personal']} < bored.personal=${bored['trinity-personal']}`)
...
function testRerankInterestStrengthDownweightsWeakPrimary() {
  ...
  // With it: 39 keeps 1.0×, 62 drops to max(0.40, 552/2119) = 0.40.
  ...
  // China should lose ~60% of its score (floor at 0.40 of tech's score, both other multipliers equal).
  assert(china._score < tech._score * 0.5,
    'rerank: weak-primary article score is well below floor × strong-primary score',
...
// ───────────────────────────────────────────────────────────────────────
// Mig 121 (2026-05-14, PR #173) — rerank prefers _redundancyAdjustedScore
// over ai_final_score when present. Audit-replay shape: pos-10 Trump-
// Beijing article had raw 880 but post-event-decay adjusted to ~195.
// Fresh Tech at raw 700 should now win.
// ───────────────────────────────────────────────────────────────────────

function testRerankPrefersRedundancyAdjustedScore() {
  const now = new Date().toISOString()
  ...
  assert(out[0].id === 2,
    'rerank: post-decay pos-10 article loses to fresh higher-quality article',
    `top=${out[0].id} decayed=${decayed._score.toFixed(2)} fresh=${fresh._score.toFixed(2)}`)
}

function testRerankFallsBackWhenAdjustedNull() {
  const now = new Date().toISOString()
  ...
  assert(out[0].id === 1,
    'rerank: no _redundancyAdjustedScore → falls back to ai_final_score',
    `top=${out[0].id}`)
}

function testRerankHonoursExplicitZeroAdjusted() {
  // ?? operator must honor 0 (no falsy fallback). An article fully
  // suppressed by redundancy decay would have adjusted ≈ 0 and should
  // lose even to lower-ai-final-score competitors.
  const now = new Date().toISOString()
  const zeroed = { id: 1, vq_primary: 39, ai_final_score: 800, _redundancyAdjustedScore: 0, created_at: now, shelf_life_days: 3 }
  const real   = { id: 2, vq_primary: 39, ai_final_score: 400, created_at: now, shelf_life_days: 3 }
  const out = rerank([zeroed, real], {})
  assert(out[0].id === 2 && zeroed._score === 0,
    'rerank: explicit 0 adjusted score is honored (no falsy bug)',
    `top=${out[0].id} zeroed=${zeroed._score} real=${real._score.toFixed(2)}`)
}
...
async function main() {
  ...
  testRerankPrefersRedundancyAdjustedScore()
  testRerankFallsBackWhenAdjustedNull()
  testRerankHonoursExplicitZeroAdjusted()
