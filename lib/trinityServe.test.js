...
function testFollowsBudgetRedistribution() {
  ...
  // The reallocation went to fresh + trinity-personal.
  assert(noFollows['trinity-fresh'] >= base['trinity-fresh'],
    'redistribute: fresh gets more slots when no follows')
  assert(noFollows['trinity-personal'] >= base['trinity-personal'],
    'redistribute: trinity-personal gets more slots when no follows')
}
...
function testSlotBudgetSnapshot() {
  ...
  // Required keys in every phase output. P2 fix (2026-05-12) replaced
  // trinity-m-tier1 + trinity-m-tier2 with a single trinity-personal pool.
  const REQUIRED_KEYS = ['trinity-fresh', 'trinity-personal', 'trinity-lt', 'trinity-follow', 'explore']
