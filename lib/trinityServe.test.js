...
function testPickRankerVersionEnvOverride() {
  ...
  try {
    // 2026-05-21 — a bare RANKER_VERSION=v1 is now IGNORED. The shipped v1
    // model has inverted coefficients (cluster_fit negative, seen_count/age
    // positive). Production had RANKER_VERSION=v1 set, which served weeks of
    // old/off-interest feeds. v1 is hard-disabled until retrained.
    process.env.RANKER_VERSION = 'v1'
    assert(pickRankerVersion('any-user') === 'v0',
      'pickRankerVersion: bare env=v1 is IGNORED (broken model hard-disabled)')
    process.env.RANKER_VERSION = 'v0'
    assert(pickRankerVersion('any-user') === 'v0',
      'pickRankerVersion: env=v0 force-off works')
    // Deliberate, self-documenting opt-in for post-retraining validation only.
    process.env.RANKER_VERSION = 'v1-allow-broken'
    assert(pickRankerVersion('any-user') === 'v1',
      'pickRankerVersion: explicit v1-allow-broken opt-in enables v1')
  } finally {
