...
// Loose interest match against a weighted Map (tag -> reading-time weight);
// returns the best matching weight (0 if none). This is how the bundles rank by
// HOW MUCH the user has read a topic, not just whether they have.
function fuzzyWeight(map, tag) {
  if (map.has(tag)) return map.get(tag) || 0;
  let best = 0;
  for (const [k, w] of map) {
    if (k.length >= 4 && tag.length >= 4 && (k.includes(tag) || tag.includes(k))) best = Math.max(best, w || 0);
  }
  return best;
}
...
export function rankBundles(bundles, user, { limit = 6, majors = 2 } = {}) {
  ...
  // user.interests is a Map(tag -> reading-time weight); user.topics is a Set.
  const interestMap = user.interests instanceof Map
    ? user.interests
    : new Map([...(user.interests || [])].map((t) => [t, 1]));
  const withScores = bundles.map((b) => {
    let personal = 0;
    const matched = [];
    for (const t of b.topicTags) {
      const w = fuzzyWeight(interestMap, t);
      if (w > 0) {
        // Weight-scaled: a heavily-read topic (high w) outranks a glanced one.
        personal += 2 + Math.min(w, 8);
        matched.push(t);
      } else if (fuzzyHas(user.topics, t)) {
        personal += 1;
        matched.push(t);
      }
