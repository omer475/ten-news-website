...
export function scoreV1(article, ctx = {}) {
  ...
  if (negativeDimensions) {
    ...
    // Mig 119: switched 'source' → 'author' axis. Keep source line for the
    // 13.5-day transition while old per-publisher rows decay out. Defensive
    // access on negativeDimensions.author — older test fixtures may pass a
    // shape without the author Map.
    const authorSkips = (article.author_id && negativeDimensions.author)
      ? (negativeDimensions.author.get(String(article.author_id)) || 0) : 0
    const sourceSkips = (typeof article.source === 'string' && negativeDimensions.source)
      ? (negativeDimensions.source.get(article.source.toLowerCase()) || 0) : 0
    P_fast_skip = clip(0.05, 0.50,
      0.05 + clusterSkips * 0.03 + primarySkips * 0.015
           + authorSkips * 0.02 + sourceSkips * 0.02)
  }
  ...
  // 6) Per-creator affinity. RealGraph-style multiplier in [0.5, 1.5].
  // Mig 119: removed the trinity-follow gate. X's RealGraph applies author
  // affinity globally — when a creator the user engages with appears in any
  // pool (personal/fresh/lt), the boost still applies. Author affinity is
  // orthogonal to the cluster-fit signal in `fit` so there's no double-count.
  let creatorAffinityMult = 1.0
  if (creatorAffinity && article.author_id) {
