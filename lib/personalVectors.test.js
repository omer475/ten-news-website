...
function makeSupabaseStub(rows, error = null) {
  return {
    rpc: async (name, args) => {
      // PR6: loader now prefers user_engagement_buffer_per_primary, with
      // auto-fallback to user_engagement_buffer when the new RPC is absent.
      if (name !== 'user_engagement_buffer' && name !== 'user_engagement_buffer_per_primary') {
