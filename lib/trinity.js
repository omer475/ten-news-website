...
export async function buildHistograms(supabase, userId) {
  // Phoenix Phase 1.6 (2026-05-08): Float64Array, not Int32. Critical bug —
  // Int32Array silently truncates assignments via ToInt32, so a fractional
  // weight (e.g. engagementWeight 0.5 or 1.5×0.5 decay = 0.75) gets stored
  // as 0. Sub-integer increments NEVER accumulated. This was already broken
  // pre-Phase-1 for partial-read weights; the Phase 1.6 time decay made it
  // catastrophic — old events decay below 1.0 and disappear entirely.
  ...
  // Phoenix Phase 1.6 (2026-05-08): widened load to lifetime events (cap 30k)
  // with exponential time decay (half-life HISTOGRAM_HALFLIFE_DAYS).
  // Replaces the previous fixed 3750-event window which buried the user's
  // long-term taste under recent explore-poisoned activity. With time decay,
  // a 30-day-old engagement counts ~50% as much as today's; a 90-day-old
  // engagement ~12.5%. Ensures lifetime top primaries (e.g. 211 events on
  // primary 39 over 36 days) survive the 432-slot explore-spillover sessions.
  const { data: events, error: evErr } = await supabase
    .from('user_article_events')
    .select('article_id, event_type, view_seconds, metadata, created_at')
    .eq('user_id', userId)
    .order('created_at', { ascending: false })
    .limit(HISTOGRAM_LIFETIME_CAP)
  ...
  const halflifeMs = HISTOGRAM_HALFLIFE_DAYS * 86400 * 1000
  // v5: collect not-interested events for histogram down-weighting + cooldown.
  const cooldownCutoffMs = nowMs - NOT_INTERESTED_COOLDOWN_HOURS * 3600 * 1000
  ...
  for (const ev of events) {
    ...
    const w = engagementWeight(ev, art)
    if (w === 0) continue
    // Phoenix Phase 1.6: exponential time decay. ageMs / halflifeMs is the
    // number of half-lives elapsed; weight decays by 2^-n.
    const evMs = new Date(ev.created_at).getTime()
...
function engagementWeight(event, article) {
  if (QUALIFYING_EVENTS.has(event.event_type)) return 1.5
