...
export async function buildHistograms(supabase, userId) {
  ...
  return { h1, h2, qualifyingCount, cooldownPrimaries, source: 'live' }
}
...
export async function retrievePersonalPerPrimary(supabase, userId, primaries, opts = {}) {
  ...
  for (const p of primaries) {
    ...
    calls.push(
      ...
      // the seen-history antijoin against user_feed_impressions. Without
      // this, the personal retriever re-served articles user had already
      // seen — audit found 100% repeat rate after PR #151.
      //
      // 2026-05-12 (mig 117): dedup window default is now lifetime (36500d
      // in the RPC). No explicit p_dedup_days override — lifetime is the
      // platform-wide policy now (matches TikTok / Instagram behavior).
...
//
// For one candidate, count how many of the user's recent fast-skipped
// articles have embedding cosine ≥ threshold (default SKIP_PENALTY_THRESHOLD).
// Return multiplier = decay^count (default SKIP_PENALTY_DECAY = 0.5).
//
// Constants and rationale documented at SKIP_PENALTY_THRESHOLD definition.
//
// Cost: O(N × 384) cosine multiplies where N = skip-embedding count. With
// N=200 and dim=384, ~77k float multiplies per candidate. At slate compose
// time we have up to ~250 candidates → ~19M ops. Fast (<50ms).
export function computeSkipPenalty(candidateEmbedding, skipEmbeddings, opts = {}) {
  if (!Array.isArray(skipEmbeddings) || skipEmbeddings.length === 0) return 1.0
  ...
  let count = 0
  for (const raw of skipEmbeddings) {
    const skipEmb = _parseEmbeddingForCluster(raw)
    if (!skipEmb) continue
    if (_cosineSim384(candEmb, skipEmb) >= threshold) count += 1
  }
  return Math.pow(decay, count)
}
...
export function applyDiversityDiscount(slate, opts = {}) {
  ...
  const axes = opts.axes || [
    // Wave 1 (2026-05-17): floor lowered 0.20 → 0.05. Audit found the 6th+
    // same-cluster card still kept 20% of its raw score (800 → 160) — high
    // enough to beat unrelated cards. X's
    // ImpressedImageClusterBasedListwiseRescoringProvider uses pure
    // (1-decay)^count with no floor. We keep a tiny 0.05 safety floor.
    { name: 'story',   fn: it => it?._storyClusterId, floor: 0.05 },
