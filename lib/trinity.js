...
// hard cut.
//
// Set to 8760 (1 year) which is effectively timeless for news content. Older
// articles get returned by ANN if they match the user's vector strongly, then
// auto-demoted to near-zero by recency decay. Bytes through ANN are cheap;
// the HNSW index returns top-K regardless of corpus size.
//
// Env override TRINITY_RETRIEVAL_WINDOW_H for fast rollback (set to 48 to
// restore the legacy cap).
export const FRESH_WINDOW_H = Math.max(
  24,
  Number(process.env.TRINITY_RETRIEVAL_WINDOW_H) || 8760
)
