...
export function adaptiveThresholds(qualifyingCount) {
  ...
  // Lowered floors (2026-05-08): the previous tP=15 floor blocked M-tier
  // for power users whose histograms are deep-but-flat (test user 5082a1df
  // had top primary count=10 — under 15, so trinityM returned empty and
  // the feed fell back to 89% explore). 3 events on a primary is a real
  // signal; threshold-floor doesn't need to be higher than that.
...
export async function buildHistograms(supabase, userId) {
  ...
  for (const ev of events) {
    ...
    // v5.1 (audit fix A3+A4, 2026-05-06): replace binary qualifying gate
    // with soft engagement weight ∈ [-0.5, +1.5]. Captures the full dwell
    // taxonomy instead of the cliff at ratio=0.5. Skips contribute small
    // negatives so the histogram learns avoidance signals (Trinity paper
    // KDD 2024 §3.2 "soft behaviour weight"). Floored at 0 downstream
    // preserves sortIndicesDesc semantics.
...
// v5.1 audit fix A3+A4. Returns ∈ [-0.5, +1.5]:
//   liked/saved/shared/revisit      → +1.5  (explicit strong positive)
//   engaged ratio ≥ 1.0              → +1.0  (full read)
//   partial 0.5 ≤ ratio < 1.0        → +0.5  (skim that finished)
//   glance 0.2 ≤ ratio < 0.5         → +0.1  (was discarded)
//   quick skip 0.05 ≤ ratio < 0.2    → −0.2  (was discarded)
//   hard skip ratio < 0.05           → −0.5  (was discarded)
...
function engagementWeight(event, article) {
  ...
  if (!expected || expected <= 0) {
    // Fallback to raw dwell tiers when expected is unknown.
    if (dwell < 1.0) return -0.5
    if (dwell < 3.0) return -0.2
    if (dwell < 6.0) return 0.1
    if (dwell < 12.0) return 0.5
    return 1.0
  }
  ...
  if (ratio < 3.00) return 1.0
  // ratio ≥ 3 — likely distracted (phone left on screen). Treat as glance.
  return 0.1
}
...
export async function retrievePersonalizedFresh(supabase, primaryClusterIds, opts = {}) {
  ...
  // Audit fix S1+S2 (2026-05-06): TikTok-style server-side antijoin against
  // the FULL impressions table when userId is provided. Replaces the 7d /
  // 2000-cap NOT IN(excludeIds) at the application layer.
  if (userId) {
    const { data, error } = await supabase.rpc('trinity_fetch_fresh_antijoin', {
      p_user_id: userId,
    ...
    console.error('[trinity] retrievePersonalizedFresh antijoin RPC failed:', error?.message)
...
// Audit fix S1+S2 (2026-05-06): use TikTok-style server-side antijoin
// against the FULL impressions table (lifetime retention), not the old
// 7-day windowed `NOT IN (excludeIds)` capped at 2000. The new RPC
// trinity_fetch_cluster_antijoin (migration 080) does NOT EXISTS against
// user_feed_impressions with the (user_id, article_id) index — O(log n)
// per candidate, no client-side cap. The bug this fixes: power users
// with 11K+ lifetime seen articles got items from 8+ days ago re-served
// when Phase 1 fix #7's 14-day adaptive tier kicked in.
//
...
async function fetchCluster(supabase, c2, sinceIso, limit, excludeIds, minScore, userId) {
  if (userId) {
    ...
    const { data, error } = await supabase.rpc('trinity_fetch_cluster_antijoin', {
      p_user_id: userId,
      p_vq_secondary: c2,
      p_hours_window: hoursWindow,
    ...
    if (!error && Array.isArray(data)) return data
    console.error(`[trinity] fetchCluster c2=${c2} antijoin RPC failed:`, error?.message)
    // Fall through to legacy path on error.
  }
  let q = supabase
    .from('published_articles')
