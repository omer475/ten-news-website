...
export const PERSONAL_POOL_WINDOW_H = Math.max(
  48,
  // 2026-06-13: 120h → 72h. The per-primary ANN orders by cosine distance and
  // truncates at the DB (age-blind), so a 5-day window let ~64% of the cosine
  // candidate universe be >48h old → day-old content padded the personal slots.
  // 72h (= 2× the ~36h recency half-life) keeps the pool recent with ample
  // supply (>1,700 articles in 0-48h alone). Env override for instant rollback.
  Number(process.env.TRINITY_PERSONAL_WINDOW_H) || 72
)
