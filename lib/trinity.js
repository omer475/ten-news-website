...
// Phoenix Phase 3.A (2026-05-08). Effort-weighted positive signals.
//
// Old: flat +1.5 weight for any explicit-positive event (like/save/share/
// revisit/detail_view). The X open-sourced "the-algorithm" engagement
// weight table (Heavy Ranker) shows reply (high effort) is weighted 27×
// favorite (low effort) — effort cost should map to signal strength.
// Translating to our text-platform action set:
//
//   article_liked        : 1.5  — low-effort double-tap, keep at baseline
//   article_detail_view  : 2.0  — source tap = curiosity + commitment
//   article_revisit      : 2.5  — intentional re-engagement (rare, strong)
//   article_saved        : 3.0  — deliberate "keep this" intent
//   article_shared       : 4.0  — friend-share = "this matters" (highest
//                                 effort positive, Instagram-confirmed
//                                 strongest signal — Mosseri Jan 2025)
//
// Negative side handled separately in the dwell tier and not_interested.
// Range top-to-bottom is 2.7× — respects effort hierarchy without one
// save/share dominating tens of reads (which would over-weight rare
// events at our scale).
export const QUALIFYING_EVENT_WEIGHTS = new Map([
  ['article_liked',       1.5],
...
// Kept for back-compat. Any code that needs a Set for membership
// (`if (QUALIFYING_EVENTS.has(eventType))`) still works.
export const QUALIFYING_EVENTS = new Set(QUALIFYING_EVENT_WEIGHTS.keys())
...
// Phoenix Phase 1.6 (2026-05-08): lowered floors from 15/5 to 3/2.
// The 15/5 floor blocked Trinity-M for power users with deep-but-flat
// histograms — top primary often had count 10-15, just under 15 floor.
// 3 events on a primary is real signal; floor doesn't need to be higher.
// Originally validated on the abandoned PR #110 branch, never made it to main.
export function adaptiveThresholds(qualifyingCount) {
  const ratio = Math.min(1, qualifyingCount / HISTOGRAM_WINDOW)
  const tP = Math.max(3, Math.round(T_P * ratio))
  const tS = Math.max(2, Math.round(T_S * ratio))
  return { tP, tS }
}
...
// Phoenix Phase 1.2a (2026-05-08). Returns ∈ [-0.5, +1.5]:
//   liked/saved/shared/revisit/detail_view → +1.5  (explicit strong positive)
//   absolute dwell ≥ 25s            → +1.5  (deep_read/absorbed — power-user signal)
//   absolute dwell 12-25s            → +1.0  (engaged_read — full read)
//   then fall through to ratio buckets for short reads
//   no dwell + non-positive event   → 0     (neutral, drop)
//
// Why the absolute-dwell floor: the previous "ratio ≥ 3 → 0.1 (distracted)"
// branch crushed the deepest readers. A user who spends 90s on a 15s article
// is the MOST engaged user, not distracted. Kuaishou's Generative Regression
// WTP (arXiv:2412.20211) buckets watch-time by duration before de-biasing —
// long-form readers must score AT LEAST as high as on-target readers. The
// new logic uses absolute-dwell sanity floors above ratio buckets.
function engagementWeight(event, article) {
  // Phoenix Phase 3.A: effort-weighted positives. Different explicit-positive
  // events get different weights based on effort cost (X engagement table
  // pattern). share > save > revisit > detail_view > like.
  const w = QUALIFYING_EVENT_WEIGHTS.get(event.event_type)
  if (w !== undefined) return w
  const dwell = pickDwellSeconds(event)
  if (dwell <= 0) return 0

  // Absolute-dwell floor: regardless of expected_read_seconds, a long read
  // is a real signal. Mirrors iOS DwellTier thresholds (deep_read ≥ 25s,
  // absorbed ≥ 45s — both treated as max positive).
  if (dwell >= 25.0) return 1.5
  if (dwell >= 12.0) return 1.0

  // Short reads (< 12s): use ratio bucketing, fall back to raw dwell if
  // expected_read_seconds is unknown.
  const expected = expectedReadSecondsForArticle(article)
  if (!expected || expected <= 0) {
    if (dwell < 1.0) return -0.5
    if (dwell < 3.0) return -0.2
    if (dwell < 6.0) return 0.1
    return 0.5  // 6-12s
  }
  const ratio = dwell / expected
  if (ratio < 0.05) return -0.5
  if (ratio < 0.20) return -0.2
...
// Kept for backwards compat — used by external callers / tests.
function isQualifying(event, article) {
  return engagementWeight(event, article) > 0
}
...
export function composeWithBudgets(pools, slateSize) {
  ...
  // Phoenix backfill: pools with hardCap=true are NEVER spilled into.
  // Personalized pools (no hardCap) get topped up first. A trending_fallback
  // pool (budget=0, hardCap=false) absorbs anything left over.
  if (out.length < slateSize) {
    for (let i = 0; i < pools.length && out.length < slateSize; i++) {
      if (pools[i].hardCap) continue
      const pool = pools[i]
...
export function applyPublisherCap(slate, opts = {}) {
  ...
  // dropped strictly. A 4th Reuters article is information-redundant with
  // the 3 already kept, not a diversity gain. The slate may end up 1-2 slots
  // short; that's preferable to publisher-heavy slates per TikTok's
  // creator-dedup rule. Trinity's other retrievers/composer can backfill.
  if (!noConsecutive) {
    return kept
  }
  ...
  return reordered  // see comment above on why no overflow concat
}
