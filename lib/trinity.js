...
export function applyDiversityDiscount(slate, opts = {}) {
  ...
  const decay = opts.decay ?? DIV_DECAY
  const floor = opts.floor ?? DIV_FLOOR
  const dropBelow = opts.dropBelow ?? 0
  // P5 fix (2026-05-12): `story` axis catches Iran-war-style topic sprawl
  // across primaries. Requires assignStoryClusters() to have stamped
  // `_storyClusterId` on each item beforehand (caller's responsibility).
  // Items without `_storyClusterId` skip this axis (defensive).
  const axes = opts.axes || [
    { name: 'story',   fn: it => it?._storyClusterId },
    { name: 'primary', fn: it => it?.vq_primary },
  ...
  for (const item of sorted) {
    ...
    for (let a = 0; a < axes.length; a++) {
      ...
      multiplier *= (1 - floor) * Math.pow(decay, prior) + floor
    }
