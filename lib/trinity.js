...
export const J_PRIMARY = 128            // # primary clusters
export const K_SECONDARY = 1024         // # secondary clusters
export const SUBCODEBOOK_K = 8          // K / J = 1024 / 128 (matches train_rq_vae.py hierarchy)
...
export async function loadActiveCodebook(supabase) {
  const { data, error } = await supabase
    .from('vq_codebooks')
    .select('id, version, signal_type, level1_centroids, level2_centroids, parent_map, dim, item_count')
    .eq('is_active', true)
  ...
  return {
    ...
    parentMap: cb.parent_map,           // length 1024
    dim: cb.dim,
    itemCount: cb.item_count,
    // level1_centroids and level2_centroids retained as-is for callers that need
    // to project new vectors. Most callers only need parent_map.
    level1: cb.level1_centroids,
    level2: cb.level2_centroids,
  }
}
