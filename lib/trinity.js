...
// Compute adaptive thresholds. The paper used T_p=30/T_s=10 for Douyin where
// users have ~2,500 qualifying behaviors. Smaller users have fewer behaviors
// per primary; preserving the paper's *ratio* (30/2500 = 1.2%) instead of the
// absolute count keeps the algorithm working at every scale.
// Lower bound 15/5 so we still require real evidence; upper bound = paper.
export function adaptiveThresholds(qualifyingCount) {
  const ratio = qualifyingCount / HISTOGRAM_WINDOW   // 1.0 if user is at paper-scale
  const tP = Math.max(15, Math.min(T_P, Math.round(T_P * ratio)))
  const tS = Math.max(5,  Math.min(T_S, Math.round(T_S * ratio)))
  return { tP, tS }
}
...
export function trinityM(h1, h2, parentMap, rng = Math.random, thresholds = null) {
  ...
  for (const c1 of primaryOrder) {
    if (h1[c1] < tP) break   // sorted, so anything below threshold is also below
