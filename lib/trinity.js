...
export function composeWithBudgets(pools, slateSize) {
  ...
  // Phoenix Phase 9.C.3 (2026-05-09): cross-pool score-floor backfill.
  //
  // Old: backfilled pools in declared order (personalized first, then trending).
  // Bug it caused: a tier1 article at score=50 took a slot while a trinity-lt
  // article at score=200 sat in overflow. Pool ordering trumped relative
  // quality. Session 2 saw this — tier1 produced 3 weak primary-224 articles
  // while LT had stronger primary 39/176 unspent items.
  //
  // New: collect EVERY remaining unseen item across non-hardCap pools, sort
  // by _score globally, take the highest-scoring ones to fill the deficit.
  // hardCap pools (explore) still respect their budget — backfill never
  // pulls from them.
  //
  // Round-robin minimum budgets are still honored above (each pool got its
  // floor); this only allocates the leftover slots by quality.
  if (out.length < slateSize) {
    const overflow = []
    for (let i = 0; i < pools.length; i++) {
      if (pools[i].hardCap) continue
      const pool = pools[i]
      for (let j = cursors[i]; j < pool.items.length; j++) {
        const item = pool.items[j]
        if (item == null || item.id == null) continue
        if (seenIds.has(item.id)) continue
        overflow.push({ item, poolIdx: i })
      }
    }
    overflow.sort((a, b) => (b.item._score || 0) - (a.item._score || 0))
    for (const { item, poolIdx } of overflow) {
      if (out.length >= slateSize) break
      if (seenIds.has(item.id)) continue
      seenIds.add(item.id)
      out.push({ ...item, _retrieverTier: pools[poolIdx].name })
      filled[poolIdx] += 1
    }
  }
