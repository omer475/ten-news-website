...
export async function retrieveFollowed(supabase, userId, opts = {}) {
  ...
  // RPC instead of a raw .from('published_articles') query. The RPC includes
  // a 7-day antijoin against user_feed_impressions — the JS-only path used
  // to only filter on iOS-supplied excludeIds, which missed older repeats.
  // Audit found 4-7 repeat follow cards per slate when this RPC didn't
  // exist.
...
export async function retrievePersonalPerPrimary(supabase, userId, primaries, opts = {}) {
  ...
  for (const p of primaries) {
    ...
    calls.push(
      ...
      // the 7-day seen-history antijoin against user_feed_impressions.
      // Without this, the personal retriever re-served articles user had
      // already seen — audit found 100% repeat rate after PR #151.
