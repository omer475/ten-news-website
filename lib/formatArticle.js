...
export function formatArticle(article, eventMap = {}) {
  ...
  const formatted = {
    ...
    // snake_case alias — TodayPlus web uses published_at to compute
    // "new since last visit". Kept alongside publishedAt for iOS back-compat.
    ...
    // Multi-page carousel. iOS Article.pages drives the swipe carousel (Pipeline 2
    // curated content has 3-8 pages; legacy page-2 has 2). Without these the app
    // collapses every article to a single page.
    pages: pages,
    format: article.format || null,
    source_type: article.source_type || null,
    // "Today's essentials" boundary flag. Default false; /api/feed/main
    // overrides true for articles in the finite daily must-know set
    // (computed in lib/essentials.js). See essentials_total in the response meta.
    is_essential: false,
    // TodayPlus redesign v1.0: structured card data (9 templates). NULL on
    // legacy articles — iOS falls back to the classic card layout.
...
export const FEED_ARTICLE_COLUMNS = [
  ...
  'pages', 'format', 'source_type', 'display',
].join(', ')
