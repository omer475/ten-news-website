...
export const TYPE = {
  ...
  // Top band — the artwork brief keeps 0–24% empty, so type sits on flat colour.
  kickerY: 0.055,
  titleTop: 0.098,      // first title baseline
  titleSize: 0.092,     // title cap height, as a fraction of width
  titleLead: 1.05,
  titleMax: 3,

  // Foot band — the brief keeps 68–100% empty.
  ruleY: 0.745,
  footTop: 0.795,       // FIRST standfirst baseline; lines flow downward
  footSize: 0.033,
  footLead: 1.42,
  footMax: 5,
  colophonY: 0.957,
  ...
  // The bands are flat now, so these are insurance against a stray subject,
  // not the thing carrying legibility. Heavy scrims just dulled the artwork.
  scrim: 0.3,
  scrimAlpha: 0.16,
  scrimTop: 0.24,
  scrimTopAlpha: 0.14,
...
/** The cover type: kicker and title in the empty top band, standfirst in the foot. */
export function drawType(ctx, w, h, story, edition) {
  ...
  // Light insurance only — the artwork leaves these bands flat and empty.
  ...
  for (;;) {
    ...
    if (lines.length <= TYPE.titleMax || size <= w * 0.05) break;
    size *= 0.93;
  }
  ...
  // Accent rule opening the foot
  ctx.strokeStyle = accent;
  ctx.lineWidth = Math.max(2, w * 0.006);
  ctx.beginPath();
  ctx.moveTo(x, h * TYPE.ruleY);
  ctx.lineTo(x + w * 0.13, h * TYPE.ruleY);
  ctx.stroke();

  // Standfirst — flows downward from the top of the foot band
  let fs = w * TYPE.footSize;
  let foot;
  for (;;) {
    ctx.font = `500 ${fs}px ${SANS}`;
    foot = wrapLines(ctx, cover.standfirst || story.dek || '', maxW);
    if (foot.length <= TYPE.footMax || fs <= w * 0.022) break;
    fs *= 0.95;
  }
  foot = foot.slice(0, TYPE.footMax);
  ctx.fillStyle = bottomInk;
  foot.forEach((line, i) => {
    ctx.fillText(line, x, h * TYPE.footTop + i * fs * TYPE.footLead);
  });
