...
export const TYPE = {
  ...
  kickerY: 0.085,       // baseline of the tag line
  titleTop: 0.135,      // first title baseline
  titleSize: 0.098,     // title cap height, as a fraction of width
  titleLead: 1.06,
  titleMax: 4,          // lines before it shrinks
  footBottom: 0.135,    // last standfirst line, measured up from the foot
  footSize: 0.038,
  footLead: 1.35,
  footMax: 4,
  ruleGap: 0.028,       // rule above the standfirst
  colophonY: 0.962,
  colophonSize: 0.024,
  scrim: 0.46,          // bottom scrim height
  scrimAlpha: 0.66,
  scrimTop: 0.42,       // top scrim height — the title lives under it
  scrimTopAlpha: 0.52,
};
...
/**
 * Ink for one band. The stored values are measured off the illustration, so
 * they only apply when there IS one — the fallback ground is always dark.
 */
export function inkFor(story, band) {
  if (!story?.art?.image_url) return '#ffffff';
  const v = band === 'top' ? story.art.topInk : story.art.bottomInk;
...
export function accentFor(story) {
  ...
  const luma = (0.2126 * r + 0.7152 * g + 0.0722 * b) / 255;
  const onDark = inkFor(story, 'top') === '#ffffff';
...
/** The cover type: kicker and title across the top, standfirst across the foot. */
export function drawType(ctx, w, h, story, edition) {
  ...
  // Scrims, so type never fights the illustration underneath it.
  const topBase = topInk === '#ffffff' ? '0,0,0' : '255,255,255';
  const topGrad = ctx.createLinearGradient(0, 0, 0, h * TYPE.scrimTop);
  topGrad.addColorStop(0, `rgba(${topBase},${TYPE.scrimTopAlpha})`);
  topGrad.addColorStop(1, `rgba(${topBase},0)`);
  ctx.fillStyle = topGrad;
  ctx.fillRect(0, 0, w, h * TYPE.scrimTop);

  const botBase = bottomInk === '#ffffff' ? '0,0,0' : '255,255,255';
  const botGrad = ctx.createLinearGradient(0, h * (1 - TYPE.scrim), 0, h);
  botGrad.addColorStop(0, `rgba(${botBase},0)`);
  botGrad.addColorStop(1, `rgba(${botBase},${TYPE.scrimAlpha})`);
  ...
  // Kicker — the one element in the artwork's own colour
  if (story.tag) {
  ...
  for (;;) {
    ...
    lines = wrapLines(ctx, cover.title || story.headline || '', maxW);
    if (lines.length <= TYPE.titleMax || size <= w * 0.052) break;
    size *= 0.93;
  }
  ...
  for (;;) {
    ...
    if (foot.length <= TYPE.footMax || fs <= w * 0.026) break;
    fs *= 0.94;
  }
  foot = foot.slice(0, TYPE.footMax);
  const footLead = fs * TYPE.footLead;
  const lastBaseline = h * (1 - TYPE.footBottom);
  ctx.fillStyle = bottomInk;
  foot.forEach((line, i) => {
    ctx.fillText(line, x, lastBaseline - (foot.length - 1 - i) * footLead);
  });

  // Accent rule above the standfirst
  const ruleY = lastBaseline - (foot.length - 1) * footLead - fs - w * TYPE.ruleGap;
  ctx.strokeStyle = accent;
  ctx.lineWidth = Math.max(2, w * 0.006);
  ctx.beginPath();
  ctx.moveTo(x, ruleY);
  ctx.lineTo(x + w * 0.13, ruleY);
  ctx.stroke();
  ...
  ctx.fillText(
    `TODAY · ${date}${edition?.issue ? ` · NO. ${edition.issue}` : ''}`,
    x, h * TYPE.colophonY,
  );
  ctx.globalAlpha = 1;
}
