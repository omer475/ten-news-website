...
// ---------------------------------------------------------------------------
// seenDecay: article seen multiple times scores lower than unseen.
function testSeenDecay() {
  const unseen = fakeArticle({ seen_count: 0 })
  const seenOnce = fakeArticle({ seen_count: 1 })
  const seen5 = fakeArticle({ seen_count: 5 })
  const s0 = scoreV1(unseen, {})
  const s1 = scoreV1(seenOnce, {})
  const s5 = scoreV1(seen5, {})
  assert(s0 > s1 && s1 > s5,
    'seenDecay: monotonically decreasing with view count',
    `s0=${s0.toFixed(3)} s1=${s1.toFixed(3)} s5=${s5.toFixed(3)}`)
}
