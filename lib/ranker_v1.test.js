...
// ---------------------------------------------------------------------------
// seenDecay: heavily-seen articles score lower than unseen.
//
...
// (seen_count proxy from max_dwell_seconds>0 in the v0 training script).
// This pushes the engagement score UP at low seen_count while the policy-
// multiplier seenDecay pushes the FINAL score down. Net effect at low
// seen_count is non-monotonic; over many views the policy mult wins.
// Asserting end-to-end behavior that matters for ranking: heavily-seen
// (5+) articles score lower than unseen. Tighten back to strict
// monotonicity once leakage-proxy is removed in future training iterations.
...
// Phase 1.3 (2026-05-11) — creatorAffinityMult applies ONLY to follow pool.
function testCreatorAffinityFollowOnly() {
  const a1 = { ...fakeArticle({ id: 10, author_id: 'pub-X' }), _retriever: 'trinity-follow' }
  const a2 = { ...fakeArticle({ id: 11, author_id: 'pub-X' }), _retriever: 'trinity-m-tier1' }
  const ctx = {
  ...
  // Same article fields, only difference is _retriever. Follow pool should
  // get the 1.5× affinity boost; other pools should not.
  assert(sFollow > sTier,
    'creatorAffinityMult: follow pool boosted; other pool unaffected',
    `follow=${sFollow.toFixed(3)} tier=${sTier.toFixed(3)}`)
}
