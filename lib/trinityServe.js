...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // 2026-05-18 — trinity-lt switched to hardCap: true.
  //
  // composeWithBudgets has a "greedy overflow" branch after round-robin that
  // pulls UNLIMITED items from non-hardCap pools when out.length < slateSize.
  // For heavy users where personal+fresh return ~0 candidates after lifetime
  // dedup (test user 5082a1df: 22k+ events seen, latest slate had 0 personal
  // + 0 fresh + 22 LT cards), the overflow filled the entire slate with
  // long-tail content from ~600 trinityLT cluster candidates. Diversity
  // discount (floor 0.25, decay 0.5) couldn't stop it because empty
  // personal+fresh pools had nothing to compete against.
  //
  // hardCap=true caps trinity-lt at budget=3, exactly like explore. When
  // personal+fresh are starved, the slate is short (e.g. 6-10 cards) but
  // NOT flooded with long-tail. The user swipes; next request retries
  // retrieval and may pull new fresh content.
  //
  // Trade-off: slates may be shorter on heavy users. Prefer-shorter over
  // prefer-irrelevant matches TikTok's "no two videos in a row from same
  // creator" floor over flooding.
  const composed = composeWithBudgets([
    ...
    { name: 'trinity-lt',       items: ltRanked,       budget: scaledBudgets['trinity-lt'] || 0, hardCap: true },
  ...
  if (slate.length < feedSize) {
    ...
    // 2026-05-18 — trinity-lt removed from backfill pool list (matches the
    // compose-step hardCap: true). Keeping LT in backfill would defeat the
    // hardCap because backfill's safety net could still pull ≤ budget+2 LT
    // cards into a slate that's already starved. Better to leave the slate
    // short than to flood it with long-tail content.
