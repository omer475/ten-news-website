// Trinity v3 serving orchestrator.
//
// Wraps the pure algorithms in lib/trinity.js with:
//   - DB calls to load codebook / cluster state / article counts
//   - Reranker (ai_final_score × content-aware recency × bucket_boost)
//   - MMR diversity rail
//   - Cluster-state EMA update on shown clusters
//   - Cold-start fallback (trending pool when histogram too sparse)
//
// v3 changes from v2:
//   v3-A. retrieveCandidatesAdaptive: 7d → 14d → 21d tiered retrieval. Heavy
//         users have exhausted their 7d interest pool (96% of last-7d articles
//         in top primaries already seen); tiered retrieval expands until the
//         pool is large enough.
//   v3-B. Content-aware recency via shelf_life_days. Half-life =
//         shelf_life_days * 24 / 2. Breaking news still decays in 12h;
//         evergreen content lasts a week. Replaces the flat 36h half-life
//         that crushed Trinity's older-but-relevant picks.
//   v3-C. Eliminated topup-trending for warm users (qualifyingCount ≥ 50).
//         Adaptive retrieval guarantees Trinity has enough; topup is the
//         cold-start spine ONLY. Stops the geopolitics-flood failure mode.
//   v3-D. Thompson Sampling exploration arm. 1 of 20 slots picks from
//         clusters where the user has h² == 0. Beta(α=1+engages, β=1+skips)
//         per cluster, sampled at request time.
//   v3-E. Soft M/LT exclusion (in lib/trinity.js trinityLT).
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // v3-A: tiered adaptive retrieval. Each retriever expands its window
  // independently until its pool is healthy.
  const [mResult, ltResult, exploreResult] = await Promise.all([
    mClusters.length
  ...
  // EMA update for shown Trinity-M / Trinity-LT clusters.
  const shownClusters = new Set(
    picked.filter(a => a._retriever === 'trinity-m' || a._retriever === 'trinity-lt')
  ...
  return {
    ...
    debug: {
      path: 'trinity-v3', qualifyingCount,
...
// v3-B: rerank uses content-aware recency derived from each article's
// shelf_life_days. Half-life = shelf_life_days * 24 / 2.
