...
// P2 fix (2026-05-12): trinity-m-tier1 + trinity-m-tier2 retired (always-empty
// pools on the live audit user). Replaced by a single `trinity-personal` pool
// fed by retrievePersonalPerPrimary (multi-vector ANN over top-10 primaries).
// Combined budget (8 slots) preserves the 8/20 = 40% personalized share.
const SLOT_BUDGETS = {
  'trinity-fresh':    3,
  'trinity-personal': 8,
  'trinity-lt':       3,
...
export function engagementAwareSlotBudgets(recentEngagementZ) {
  if (recentEngagementZ > 0.5) {
    return {
      'trinity-fresh':    3,
      'trinity-personal': 10,  // 6+4 combined — exploit hard
      'trinity-lt':       2,
      'trinity-follow':   3,
      'explore':          2,
    }
  ...
  if (recentEngagementZ < -0.5) {
    return {
      'trinity-fresh':    3,
      'trinity-personal': 6,   // 3+3 combined — bored, broaden
      'trinity-lt':       3,
      'trinity-follow':   3,
      'explore':          5,
    }
...
export function redistributeFollowsBudget(budgets, hasFollows) {
  ...
  // Split follow slots: 60% to fresh (recency is a strong signal for users
  // without follows), 40% to trinity-personal (the strongest personalized
  // lane). Pre-P2 the 40% went to trinity-m-tier1.
  const toFresh = Math.ceil(followSlots * 0.6)
  const toPersonal = followSlots - toFresh
  out['trinity-fresh'] = (out['trinity-fresh'] || 0) + toFresh
  out['trinity-personal'] = (out['trinity-personal'] || 0) + toPersonal
  return out
}
...
export function phaseAwareSlotBudgets(qualifyingCount, recentEngagementZ) {
  ...
  if (qualifyingCount < EXPLORE_PHASE_THRESHOLDS_QC[0]) {
    return {
      'trinity-fresh':    3,
      'trinity-personal': 6,   // 3+3 combined
      'trinity-lt':       3,
      'trinity-follow':   2,
      'explore':          6,
    }
  ...
  if (recentEngagementZ > 0.5) {
    return {
      'trinity-fresh':    3,
      'trinity-personal': 9,   // 5+4 combined
      'trinity-lt':       2,
      'trinity-follow':   3,
  ...
  return {
    'trinity-fresh':    3,
    'trinity-personal': 7,   // 4+3 combined
    'trinity-lt':       2,
    'trinity-follow':   3,
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const [hourPrimaryBoosts, negativeDimensions, userFunnelStats, userActionRates, userFollowedAuthors, userOnboardingTags, creatorAffinity, sessionExposure] = await Promise.all([
  ...
  // sampling with multi-vector ANN retrieval per top-10 primary (see
  // retrievePersonalPerPrimary in lib/trinity.js). Reason: the old M-tier
  // path returned 0 articles for the test user despite high histogram
  // weight, because the random secondary-cluster pick rarely matched what
  // was actually published in the last 48h. Spec section 3.2 locks the
  // replacement to Trinity (KDD 2024) N_M=10 + PinnerSage user vectors.
  //
  // Personal primaries = top-10 by h¹, minus cooldowns. Avoid-cluster set
  // is filtered article-side in the ANN RPC by primary, not secondary —
  // that's a known coverage gap (basketball-within-Sports may slip
  // through if a Sports primary stays unblocked). The diversity discount
  // and ENF dimension penalties downstream catch that case.
  const personalPrimaries = topPrimariesFromHistogram(h1, N_M)
  ...
  let [personalPool, ltResult, exploreResult, freshPool, trendingFallback, followPool] = await Promise.all([
    // P2 fix (2026-05-12). Multi-vector ANN per top-10 primary. Returns
    // up to 10 × perPrimaryLimit candidates; light rank + dedup downstream.
    ...
    // Phase 1.1 (2026-05-11): histogram-fit filter on top-20 primaries by
    // weight (NOT cooldown-filtered — followed creators were explicitly
    // chosen by the user, so a Not-Interested cooldown on the primary
    // shouldn't suppress their content). Catches the firehose-publisher
    // case where a single high-volume creator dumps off-taste articles.
    // Tight-filter fallback below in case the filter starves the pool.
    userId
      ? retrieveFollowed(supabase, userId, {
          excludeIds: seenIds, hoursWindow: 7 * 24, minScore: 200, limit: 30,
          onlyPrimaries: freshPrimariesAll.slice(0, 20),
        })
      : Promise.resolve([]),
  ])

  // Phase 1.1 (2026-05-11): tight-filter fallback for trinity-follow. When
  // the histogram filter excludes nearly all of a user's followed-publisher
  // articles, refetch unfiltered so users with narrow histograms but diverse
  // follows aren't left empty-handed. Emit a telemetry log so this case is
  // observable.
  if (userId && Array.isArray(followPool) && followPool.length < 5) {
    console.log(
      `[trinity.follow] tight_filter user=${userId.slice(0, 8)} ` +
      `filtered=${followPool.length} top20Set=${freshPrimariesAll.slice(0, 20).length}`
    )
    const unfiltered = await retrieveFollowed(supabase, userId, {
      excludeIds: seenIds, hoursWindow: 7 * 24, minScore: 200, limit: 30,
    })
    if (Array.isArray(unfiltered) && unfiltered.length > followPool.length) {
      followPool = unfiltered
    }
  }
  ...
  // trinity-personal can return up to N_M × perPrimaryLimit = 200 candidates;
  // light-rank prune to 60 (slightly higher than other pools because it
  // covers 10 primaries — more diverse content survives the cut).
  personalPool       = lightRank(personalPool,       lightCtx, 60)
  ltResult           = { ...ltResult,     pool: lightRank(ltResult.pool,     lightCtx, 50) }
  trendingFallback   = lightRank(trendingFallback,   lightCtx, 50)
  // exploreResult and followPool deliberately skipped.
  ...
  const rerankOpts = rankerVersion === 'v1'
    ...
    : { recentEventIds, categoryMultipliers, hourPrimaryBoosts, negativeDimensions, userFunnelStats, todaysPrimaryCounts, creatorAffinity }
  ...
  // Fix B (2026-05-12): apply 3-axis session-exposure multiplier to every
  // ranked candidate. Multipliers: 1.0× when fresh on all 3 axes (primary,
  // secondary, source). Heavy decay when over-served.
  // Source: Kuaishou tri-level fatigue (WTG CIKM 2023). Same shape every
  // major platform uses for cross-session topic memory.
  ...
  applyExposure(exploreRanked); applyExposure(trendingRanked); applyExposure(followRanked)
  ...
  // online single-linkage clustering on `embedding_minilm_vec`. Used as a
  // diversity axis in both composeWithBudgets backfill AND
  // applyDiversityDiscount. Catches topic sprawl across primaries (e.g.
  // Iran-war coverage that scatters across 5 vq_primary clusters).
  // Source: Google News / Apple News pattern; X open-source content-
  // embedding diversity stage.
  const allRankedForCluster = [
    ...freshRanked, ...personalRanked, ...ltRanked,
    ...followRanked, ...exploreRanked, ...trendingRanked,
  ]
  assignStoryClusters(allRankedForCluster)

  // P6 fix (2026-05-12): hardCap=true on trinity-follow.
  // Matches Instagram main feed, Threads For You, YouTube Home, IG Reels:
  // followed content lives in a separate Following tab (iOS work) and the
  // For You feed caps follow share at ~15% (3 of 20 slots). The old
  // soft-cap let follow back-fill personal's empty slots when personal
  // came under-budget, producing 60% follow share on the audit user.
  // Per WebConf 2024 TikTok audit + IG transparency report 2023.
  // Fix D (2026-05-12): hardCap on trending-fallback at 2 items. Was
  // budget=0 but with no hardCap the backfill loop pulled 6/25 in one
  // audit slate when personal pool came under-budget. Every major
  // platform strictly caps fallback content (TikTok 10-15%, IG 20%,
  // X 30-50% — never unbounded).
  const composed = composeWithBudgets([
    ...
    { name: 'trinity-follow',    items: followRanked,   budget: scaledBudgets['trinity-follow'] || 0, hardCap: true },
    { name: 'explore',           items: exploreRanked,  budget: scaledBudgets['explore'] || 0, hardCap: true },
    { name: 'trending-fallback', items: trendingRanked, budget: 2, hardCap: true },
  ], overFetchTarget)

  // P1+P3 fix (2026-05-11): X-style soft diversity discount on (primary,
  // source) axes. Replaces the hard MAX_PER_PRIMARY=4 / MAX_PER_PUBLISHER=3
  // caps with the open-source X formula `score *= (1-floor)*decay^prior+floor`
  // with decay=0.5, floor=0.25. After 3 articles from same primary, the 4th
  // is at 0.34× score; the 5th at 0.25× (asymptote). Same shape on source
  // axis. The compose-time backfill (lib/trinity.js composeWithBudgets) has
  // already applied the SAME discount on (pool, primary, source) axes to
  // the overflow items, so by the time we get here the slate is mostly
  // diversified. This pass is a final re-discount + re-rank of the full
  // slate so round-robin items also get the smooth attenuation.
  //
  // Source: twitter/the-algorithm DiversityDiscountProvider.scala.
  // Fixes:
  //   * P1 — trinity-follow 45% → ≤ ~18% (the 3rd+ follow items get 0.34× score)
  //   * P3 — Iran-topic 33% → ≤ ~12% (the 4th+ same-primary items get 0.34×)
  let slate = applyDiversityDiscount(composed)

  // Story-cluster cap (1 per world_event_id). Strict drop — duplicate
  // stories add no information. Google News pattern. Kept as hard cap
  // because the discount can't capture "same event different angle" with
  // sufficient strength.
  //
  // 2026-05-12: moved BEFORE pinPersonalToTopSlots. Earlier order
  // (pin then cap) could drop a pinned personal card if its event_id
  // was already in top-N, silently breaking the "≥2 personal in top-5"
  // guarantee. Capping first means pin operates on a clean slate.
  slate = applyEventCap(slate)

  // Fix C (2026-05-12): pin at least 2 trinity-personal cards into the top
  // 5 slots. Without this, personal cards land at slots 17-22 after the
  // discount sort and the user bounces before seeing them (audit: user
  // skipped 5 of 7 opening cards in their last session before any AI/tech
  // personal content appeared).
  // Source: TikTok Trinity paper Algorithm 3 "slot position policy";
  // Instagram position-aware re-ranking; Pinterest Pinnability position
  // calibration; X RescoreProvider position prior.
  slate = pinPersonalToTopSlots(slate, { minPersonal: 2, topN: 5 })
  ...
  //
  // 2026-05-12: removed trendingRanked + followRanked from the backfill
  // list. Both are hardCap pools at compose time (lines above) — letting
  // them spill into backfill by raw _score blew the cap to 15-18 follow
  // cards in audits because follow articles score 800-905 (creator-affinity
  // boost). The function comment above stated the intent already; the
  // call site was contradicting it. exploreRanked was already excluded.
  slate = mmrDiversify(slate, feedSize)
  if (slate.length < feedSize) {
    slate = backfillToTarget(slate, [ltRanked, personalRanked, freshRanked], feedSize)
  }
  ...
  const personalized = new Set(['trinity-personal', 'trinity-lt', 'trinity-fresh', 'trinity-follow'])
...
// runs BEFORE the heavy rerank() / scoreV1() for each large pool. Reduces
// 60-200 candidates per pool to topN (default 50) so the heavy ranker (which
// will become a learned model in Phase 2.4) sees ~50% fewer rows at constant
// quality. Mirrors X's 3-stage architecture: retrieval → light → heavy.
//
...
export function lightRank(candidates, ctx = {}, topN = 50) {
  ...
  candidates.sort((a, b) => (b._lightScore || 0) - (a._lightScore || 0))
...
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    // Phase 1.3 (2026-05-11) — per-creator affinity. RealGraph-style boost
    // for followed publishers the user actually engages with; demote for
    // followed publishers they ignore. Only applied to trinity-follow pool;
    // other pools are scoped by VQ cluster, not author.
    let creatorAffinityMult = 1.0
    if (creatorAffinity && a._retriever === 'trinity-follow' && a.author_id) {
    ...
    a._score = qualityNorm * recency * eventDemote * catMult * seenMult * hourMult * negMult * funnelMult * todayPenalty * creatorAffinityMult
  }
