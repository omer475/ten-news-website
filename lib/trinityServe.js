...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const rerankOpts = rankerVersion === 'v1'
    ...
    : { categoryMultipliers, hourPrimaryBoosts, negativeDimensions, userFunnelStats, todaysPrimaryCounts, creatorAffinity, h1, h2, skipEmbeddings }
...
export function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    a._score = qualityNorm * recency * catMult * seenMult * hourMult * negMult * funnelMult * todayPenalty * creatorAffinityMult * interestStrength * interestStrengthH2 * skipPenalty
  }
