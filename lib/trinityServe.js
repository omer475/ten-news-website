...
//   +1 = all engages in last N        → push exploration
//
// Empirical bands (mirrors Vombatkere TikTok audit ranges 30-50% explore
// for typical sessions, 50-70% for engaged power users — though we
// translate "more explore" as a 30% lift not a 70% slate, which would
// be inappropriate for our smaller feed size):
//   z < −0.5  : explore 4→1, tier1 5→7, tier2 4→4 (push known-good)
//   z > +0.5  : explore 4→6, tier1 5→4, tier2 4→3 (push exploration)
...
function engagementAwareSlotBudgets(recentEngagementZ) {
  if (recentEngagementZ < -0.5) {
    return {
      ...
      'trinity-m-tier1': 7,
      'trinity-m-tier2': 4,
      'trinity-lt':      4,  // pulled-up by 1 to absorb the explore loss
      'explore':         1,
    }
  }
  if (recentEngagementZ > 0.5) {
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  let [mTier1Result, mTier2Result, ltResult, exploreResult, freshPool] = await Promise.all([
  ...
  const baseBudgets = engagementAwareSlotBudgets(recentEngagementZ)
...
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    a._score = qualityNorm * recency * bucketBoost * stale * eventDemote * catMult * pubMult
  }
...
function bumpExploreShows(supabase, clusterIds) {
  ...
  supabase.rpc('bump_cluster_explore_shows', { p_cluster_ids: clusterIds })
    .then(({ error }) => {
