...
export function engagementAwareSlotBudgets(recentEngagementZ) {
  // 2026-05-13: composition cleanup retired trinity-follow + trending-fallback.
  // Sum still = 20. Follow's old 3 slots → personal+fresh (where the
  // creator-affinity boost will surface followed creators when their content
  // matches the user's taste).
  if (recentEngagementZ > 0.5) {
    return {
      'trinity-fresh':    4,
      'trinity-personal': 12,  // exploit hard
      'trinity-lt':       2,
      'explore':          2,
    }
  }
  if (recentEngagementZ < -0.5) {
    return {
      'trinity-fresh':    4,
      'trinity-personal': 8,   // bored, broaden
      'trinity-lt':       3,
      'explore':          5,
    }
  }
...
export function phaseAwareSlotBudgets(qualifyingCount, recentEngagementZ) {
  // 2026-05-13: composition cleanup retired trinity-follow + trending-fallback.
  // All four shapes still sum to 20 = FINAL_FEED_SIZE. Follow slots redistributed
  // to personal (engages at 57% post-mig-119) and fresh (engages at 70%).
  //
  // Phase 1: new user — 30% explore.
  ...
  // Phase 2: mid user — defer to engagement-Z logic.
  if (qualifyingCount < EXPLORE_PHASE_THRESHOLDS_QC[1]) {
    return engagementAwareSlotBudgets(recentEngagementZ)
  }
  // Phase 3: heavy user. Vombatkere et al. WebConf 2024: TikTok exploit fraction
  // 30-50% even for engaged heavy users. Engagement-Z shifts ±2-3 slots.
  if (recentEngagementZ > 0.5) {
    return {
      'trinity-fresh':    4,
      'trinity-personal': 11,
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const [
    { h1, h2, qualifyingCount, cooldownPrimaries: histogramCooldownPrimaries, source: histogramSource },
    clusterState,
    articleCounts,
  ...
  // Mig 121 — attach redundancy-adjusted scores BEFORE lightRank so the
  // prune step uses post-decay quality. Otherwise 50+ Trump-Beijing wire-
  // pickups could fill the top-60 personal pool after lightRank, then
  // rerank's redundancy-aware quality only ranks within that already-
  // overflooded set. Reading on the union of all 4 raw pools is one
  // batched query — small extra DB roundtrip but keeps the prune honest.
  const allRetrieved = [
    ...freshPool, ...personalPool,
    ...ltResult.pool, ...exploreResult.pool,
  ]
  await attachRedundancyScores(supabase, allRetrieved)
  ...
  // Event-id attach for cross-session demote + applyEventCap. The
  // redundancy-score attach already ran above (before lightRank).
  ...
  const rerankOpts = rankerVersion === 'v1'
    ...
    : { recentEventIds, categoryMultipliers, hourPrimaryBoosts, negativeDimensions, userFunnelStats, todaysPrimaryCounts, creatorAffinity, h1 }
  ...
  // applyDiversityDiscount. Catches topic sprawl across primaries.
  ...
  if (slate.length < feedSize) {
    // 2026-05-13: rewritten as pool-priority fill (was raw-score sort, which
    // let LT win backfill slots even when personal had higher-priority).
    // Order: personal first to its deficit, then fresh, then lt.
...
export function lightRank(candidates, ctx = {}, topN = 50) {
  ...
  for (const a of candidates) {
    // Mig 121 — match the rerank read site so lightRank's prune step also
    // uses post-event-cluster decay quality. Same fallback shape.
    const quality = Math.log1p(Number(a._redundancyAdjustedScore ?? a.ai_final_score ?? 0) / 100)
...
//   interestStrength — h1[primary]/max(h1), floor 0.40 (added 2026-05-14)
export function rerank(candidates, opts = {}) {
  ...
  //     importance-weighted cluster sampling.
  //   * Trinity (KDD 2024, arxiv:2402.02842) Algorithm 2 — long-tail sampler
  //     uses (β + h²)^α weighting. We already use this shape in trinityLT
  //     (lib/trinity.js:615); now we use it in score too.
  // Floor 0.40 so weak-but-qualifying primaries get attenuated but not crushed —
  // a 26%-of-max primary lands at 0.40, not 0.26. The user may still want
  // occasional coverage of a small interest when nothing better is available.
  const h1 = opts.h1 && opts.h1.length ? opts.h1 : null
  let maxH1 = 0
  if (h1) for (let i = 0; i < h1.length; i++) if (h1[i] > maxH1) maxH1 = h1[i]
  const INTEREST_STRENGTH_FLOOR = 0.40
  for (const a of candidates) {
    const recency = recencyWeightForArticle(a, now)
    // Mig 121: prefer redundancy-adjusted score (post-event-cluster decay)
    // when present. Falls back to ai_final_score for articles outside the
    // 72h decay window or with no event_id. The adjusted score is computed
    // by complete_clustered_8step_workflow.py step 13 after step6 world-
    // event detection. X-style decay: (1-floor)*decay^pos+floor with
    // decay=0.5, floor=0.25 — top-of-cluster article keeps full score; the
    // 2nd drops to 0.625×, 3rd to 0.44×, asymptote 0.25×.
    const quality = Number(a._redundancyAdjustedScore ?? a.ai_final_score ?? 0)
    let eventDemote = 1
    ...
    a._score = qualityNorm * recency * eventDemote * catMult * seenMult * hourMult * negMult * funnelMult * todayPenalty * creatorAffinityMult * interestStrength
  }
...
function backfillToTarget(slate, pools, feedSize) {
  ...
  // Final safety net: if we still haven't reached feedSize, fill from ANY
  // remaining pool item in priority order regardless of budget.
...
// 2026-05-14 (mig 121). attachRedundancyScores: in-place set
// candidate._redundancyAdjustedScore from a single batched lookup against
// published_articles.redundancy_adjusted_score (X-style event-cluster
// decay, written by complete_clustered_8step_workflow.py step 13).
//
// Reading separately rather than including the column in every retrieval
// RPC's RETURNS TABLE so the migration footprint stays tight (mig 121
// only touches one column + one function; no RPC-recreation chain).
// Cost: one batched SELECT per slate request, ~10ms.
//
// Articles outside the 72h decay window have NULL adjusted_score; the
// rerank read site uses `?? a.ai_final_score` to fall back.
async function attachRedundancyScores(supabase, candidates) {
  ...
  try {
    const CHUNK = 500
    const map = new Map()  // article_id → redundancy_adjusted_score
    for (let i = 0; i < ids.length; i += CHUNK) {
      const slice = ids.slice(i, i + CHUNK)
      const { data, error } = await supabase
        .from('published_articles')
        .select('id, redundancy_adjusted_score')
        .in('id', slice)
      if (error) continue
      for (const row of (data || [])) {
        if (row.redundancy_adjusted_score != null) {
          map.set(row.id, Number(row.redundancy_adjusted_score))
        }
      }
    }
    for (const a of candidates) {
      if (a && a.id != null && map.has(a.id)) {
        a._redundancyAdjustedScore = map.get(a.id)
      }
    }
  } catch (err) {
    console.error('[trinity] attachRedundancyScores failed:', err.message)
  }
}
