...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const personalPrimaries = primariesAboveThreshold(h1, thresholds.tP, 50)
  ...
  // Mig 119 (2026-05-13): retired the top-3-vs-overflow split. With a uniform
  // FRESH_MIN_SCORE=300 floor and all interest primaries eligible, everything
  // returned by retrievePersonalizedFresh is trinity-fresh quality. The 7-day
  // trending-fallback pool stays separate as the deeper backup (mig 119
  // doesn't touch that retrieval). No-op when freshPool is empty.
  ...
  if (slate.length < feedSize) {
    // Mig 119 (2026-05-13): reordered from [lt, personal, fresh] to
    // [personal, fresh, lt]. The old order let trinity-lt flood the slate
    // whenever MMR dropped cards, because LT articles have high raw scores
    // (they're niche topics with strong historical engagement) and won the
    // backfill by score. Audit on test user showed LT filling 14-17 of 25
    // slots despite a 3-slot budget. Personal (37% engage) and fresh (40%
    // engage) consistently out-perform LT (22% engage) — they should fill
    // the backfill first. Matches Instagram / TikTok / Pinterest priority:
    // personalized → fresh → niche, never niche-first.
    slate = backfillToTarget(slate, [personalRanked, freshRanked, ltRanked], feedSize)
  }
