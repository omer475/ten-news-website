...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const recentEventIds = await loadRecentEventIds(supabase, userId)
  ...
  const freshRanked = rerank(freshPool, { recentEventIds })
  const mTier1Ranked = rerank(mTier1Result.pool, { recentEventIds })
  const mTier2Ranked = rerank(mTier2Result.pool, { recentEventIds })
  const ltRanked = rerank(ltResult.pool, { recentEventIds })
  const exploreRanked = rerank(exploreResult.pool, { recentEventIds })
...
// Optional opts.recentEventIds: Set<event_id> of stories the user already
// saw recently — articles in those events get a 0.3× cross-session demote
// (Google News story-cluster pattern; v5.1 Phase 1 fix #2).
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    a._score = quality * recency * bucketBoost * stale * eventDemote
  }
