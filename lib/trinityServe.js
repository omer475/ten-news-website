...
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    // Phoenix Phase 6.C — multi-granular negative feedback. Each matched
    // dimension contributes a small penalty. Cluster=0.05/skip, primary=
    // 0.02/skip, source=0.04/skip. Capped at 0.5 multiplier (50% reduction).
    let negMult = 1
    if (negDims) {
      let penalty = 0
      if (a.vq_secondary != null) {
        penalty += 0.05 * (negDims.cluster.get(String(a.vq_secondary)) || 0)
      }
      if (a.vq_primary != null) {
        penalty += 0.02 * (negDims.primary.get(String(a.vq_primary)) || 0)
      }
      if (typeof a.source === 'string') {
        penalty += 0.04 * (negDims.source.get(a.source.toLowerCase()) || 0)
      }
      if (penalty > 0) negMult = Math.max(0.5, 1 - penalty)
    }
