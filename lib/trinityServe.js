...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const rerankOpts = rankerVersion === 'v1'
    ...
    : { categoryMultipliers, hourPrimaryBoosts, negativeDimensions, userFunnelStats, todaysPrimaryCounts, creatorAffinity, h1, skipEmbeddings }
...
export function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    a._score = qualityNorm * recency * catMult * seenMult * hourMult * negMult * funnelMult * todayPenalty * creatorAffinityMult * interestStrength * skipPenalty
  }
...
// for guest users.
// 2026-05-17 (Wave 9). loadRecentSkipEmbeddings — last 200 fast-skipped
// or explicitly-not-interested article embeddings. Input to rerank()
// skipPenalty multiplier. Pattern from X InteractionGraphNegativeJob
// (aggregates 5 negative signal types into a per-user negative graph);
// we collect skipped article embeddings as the "skip memory."
//
// Both 'article_skipped' (dwell < 3s) and 'article_not_interested' (long-
// press hard reject) count. Older events fall out naturally as the user
// generates new ones (rolling-200 buffer).
//
// Returns array of raw embedding values for parsing inside computeSkipPenalty.
async function loadRecentSkipEmbeddings(supabase, userId, limit = 200) {
  ...
  try {
    const { data: skips, error: e1 } = await supabase
      ...
      .select('article_id')
      .eq('user_id', userId)
      .in('event_type', ['article_skipped', 'article_not_interested'])
      .order('created_at', { ascending: false })
      .limit(limit)
    if (e1 || !skips || skips.length === 0) return []
    const articleIds = Array.from(new Set(
      skips.map(r => r.article_id).filter(Boolean)
    ))
