...
export async function serveTrinityFeed(supabase, opts) {
  ...
  if (qualifyingCount < COLD_START_QUALIFYING_FLOOR && !synthDebug.applied) {
    ...
    // Category cap BEFORE the pick so the slate isn't ~68% World+Politics.
    // ceil(0.35 × 20) = 7 max per category group; overflow still backfills.
    const diversified = capByCategory(ranked, Math.ceil(0.35 * feedSize))
