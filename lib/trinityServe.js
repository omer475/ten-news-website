...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // (cluster / source / primary). Run in parallel.
  const [hourPrimaryBoosts, negativeDimensions] = await Promise.all([
  ...
  const rerankOpts = { recentEventIds, categoryMultipliers, publisherPenalties, hourPrimaryBoosts, negativeDimensions }
...
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    a._score = qualityNorm * recency * bucketBoost * stale * eventDemote * catMult * pubMult * seenMult * hourMult * freshMult * negMult
  }
