...
import { synthesizeWarmStartHistogram } from './coldStart.js'
import {
  buildHistograms,
  trinityM,
  trinityLT,
...
//
// Phase 2.1: trinity-follow gets 1.5 — follows are a strong intent signal,
// articles in this pool deserve high intra-pool ranking.
const BUCKET_BOOST = {
  'trinity-fresh':   1.6,
  'trinity-m-tier1': 1.5,
  'trinity-follow':  1.5,
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // Phoenix Phase 6.B (2026-05-09): time-of-day primary boost.
  // Phoenix Phase 6.C (2026-05-09): multi-granular negative feedback.
  // Phase 1.3 (2026-05-10): per-primary median dwell — kept for back-compat
  // but superseded by funnelStats below. Both run; rerank uses funnelStats
  // when available, falls back to dwellMult when funnelStats has < 5 events.
  // Phase 2.2 (2026-05-10): per-primary tap-then-read funnel stats.
  // Two-stage signal (P(tap) × P(deep_read | tap)) is a strictly richer
  // signal than median dwell alone.
  const [hourPrimaryBoosts, negativeDimensions, userMedianDwellByPrimary, userFunnelStats] = await Promise.all([
    loadHourPrimaryBoosts(supabase, userId),
    loadNegativeDimensions(supabase, userId),
    loadUserMedianDwellByPrimary(supabase, userId),
  ...
  let [mTier1Result, mTier2Result, ltResult, exploreResult, freshPool, trendingFallback, followPool] = await Promise.all([
    ...
    explClusters.length
      ...
      : Promise.resolve({ pool: [], hoursWindow: 0 }),
    // Phase 1.4 (2026-05-10): lower fresh score floor for top-3 primaries
    // (300) than for ranks 4-10 (500). Expands supply for the user's
    // strongest interests without polluting the long tail. Replaces the
    // mig 099 dedup-shrink hack — fixes the supply problem, not the
    // dedup window.
    freshPrimaries.length
      ? Promise.all([
          // Top-3: lower floor (300) for max supply.
          retrievePersonalizedFresh(supabase, freshPrimaries.slice(0, 3), {
            excludeIds: seenIds, limit: FRESH_POOL_LIMIT, userId, minScore: 300,
          }),
          // Ranks 4-10: standard floor (FRESH_MIN_SCORE = 500).
          freshPrimaries.length > 3
            ? retrievePersonalizedFresh(supabase, freshPrimaries.slice(3), {
                excludeIds: seenIds, limit: FRESH_POOL_LIMIT, userId,
              })
            : Promise.resolve([]),
        ]).then(([top3, rest]) => [...top3, ...rest])
      : Promise.resolve([]),
  ...
  // Phoenix Phase 7.B (2026-05-09): fresh-item boost for the EXPLORE pool only.
  // Source: Google YouTube Shorts 2025 (arxiv:2505.09033) — when explore picks
  // a cluster, cold (rarely-impressed-globally) articles get a boost so new
  // content gets its "test audience" stage. Doesn't apply to personalized
  // pools where AI score + recency are already strong signals.
  const exploreImpressionBoosts = await loadGlobalImpressionBoosts(supabase, exploreResult.pool)
  ...
  const rerankOpts = { recentEventIds, categoryMultipliers, publisherPenalties, hourPrimaryBoosts, negativeDimensions, userMedianDwellByPrimary, userFunnelStats, todaysPrimaryCounts }
  ...
  const exploreRanked = rerank(exploreResult.pool, { ...rerankOpts, freshItemBoosts: exploreImpressionBoosts })
  const trendingRanked = rerank(trendingFallback, rerankOpts)
  const followRanked = rerank(followPool, rerankOpts)
...
function rerank(candidates, opts = {}) {
  ...
  // Phoenix Phase 7.B (2026-05-09): fresh-item boost (explore pool only).
  const freshItemBoosts = opts.freshItemBoosts instanceof Map ? opts.freshItemBoosts : null
  // Phoenix Phase 6.C (2026-05-09): multi-granular negative feedback.
  const negDims = opts.negativeDimensions || null
  // Phase 1.3 (2026-05-10): per-primary median dwell. Replaces mig 097's
  // histogram-side watch-time bump. Boosts candidates from clusters where
  // the user reads deeply, demotes clusters they only tap-and-bounce.
  const userMedianDwellByPrimary = opts.userMedianDwellByPrimary instanceof Map
    ? opts.userMedianDwellByPrimary
    : null
  // Phase 2.3 (2026-05-10): same-tag-today penalty applied GLOBALLY.
  const todaysPrimaryCounts = opts.todaysPrimaryCounts instanceof Map
    ? opts.todaysPrimaryCounts
    : null
  // Phase 2.2 (2026-05-10): per-primary tap-then-read funnel stats.
  // Map<vq_primary, { impressions, taps, deep_reads }>. When set, takes
  // precedence over userMedianDwellByPrimary (richer signal).
  ...
  for (const a of candidates) {
    ...
    // Phoenix Phase 6.B (LIC-lite) — current-hour primary boost in [1.0, 1.3].
    let hourMult = 1
    if (hourPrimaryBoosts && a.vq_primary != null) {
      const m = hourPrimaryBoosts.get(a.vq_primary)
      if (m != null) hourMult = m
    }
    // Phoenix Phase 7.B — fresh-item boost in explore (cold articles preferred).
    let freshMult = 1
    if (freshItemBoosts && a.id != null) {
      const m = freshItemBoosts.get(Number(a.id))
      if (m != null) freshMult = m
    }
    // Phoenix Phase 9.A.2 (2026-05-09): strengthen ENF.
    //
    // Old (Phase 6.C): summed per-skip multipliers floored at 0.5×.
    // 6 fast-skips on primary 147 → only 12% penalty. Cluster 1798 with
    // 2 skips → 0% penalty (filtered out by p_min_count=3).
    //
    // New: multiplicative logistic decay per dimension. Each dimension
    // produces 1/(1 + skipCount × k) which:
    //   * starts at 1.0 (no skips)
    //   * smoothly drops as skips accumulate (no cliff)
    //   * scales asymptotically toward 0 (strong rejection → near-zero score)
    //   * NO floor cap — clear rejections should be near-eliminated
    //
    // Rates picked so 1 skip = 33% off cluster, 23% off primary, 28% off
    // source. 6 skips = 75% off cluster.
    // Multiplied across dimensions → "bad cluster from a bad source"
    // gets compounded penalty, matching user reality.
    let negMult = 1
    if (negDims) {
    ...
    // Phase 1.3 (2026-05-10): per-primary dwell multiplier. Where the user
    // reads deeply, candidates from that primary score higher. Where they
    // only tap-and-bounce, candidates get demoted. Bounded [0.5, 2.0].
    //
    // ratio = userMedian / max(5, expected * 0.4)
    //   numerator = the user's typical dwell on this primary (signal of intent)
    //   denominator = a fraction of the article's expected reading time, with a
    //                 5-second floor so very-short articles don't inflate the
    //                 ratio. expected*0.4 means "did they get past the first
    //                 ~40% of the article" as the engagement threshold.
    //
    // Example: user median on primary X is 18s, article expected_read 45s
    //   → ratio = 18 / max(5, 18) = 1.0 → dwellMult = 1.0 (neutral)
    // Example: user median on primary X is 35s (deep read), expected 30s
    //   → ratio = 35 / max(5, 12) = 2.9 → clamped to 2.0 (boost)
    // Example: user median on primary X is 4s (quick taps), expected 30s
    //   → ratio = 4 / max(5, 12) = 0.33 → clamped to 0.5 (demote)
    // Phase 2.2 (2026-05-10): tap-then-read funnel multiplier.
    // Empirical-Bayes-shrunk per-primary tap rate × deep-read rate.
    // Beta(5, 20) prior on tapRate (~20% baseline); Beta(2, 6) prior on
    // deepReadRate (~25% baseline given a tap). Bounded [0.7, 1.6].
    //
    // Falls back to median-dwell mult when no funnel data (cold user).
    let dwellMult = 1.0
    ...
    if (fs && fs.impressions >= 5) {
      ...
      dwellMult = Math.max(0.7, Math.min(1.6, (tapShrunk / 0.20) * (readShrunk / 0.30)))
    } else if (userMedianDwellByPrimary && a.vq_primary != null) {
      // Fallback: median-dwell ranker (Phase 1.3).
      const userMedian = userMedianDwellByPrimary.get(a.vq_primary)
      const expected = Number.isFinite(a.expected_read_seconds) && a.expected_read_seconds > 0
        ? a.expected_read_seconds : 30
      if (userMedian != null) {
        const ratio = userMedian / Math.max(5, expected * 0.4)
        dwellMult = Math.max(0.5, Math.min(2.0, ratio))
      }
    }
    // Phase 2.3 (2026-05-10): same-tag-today penalty (TikTok Algo 101 leak).
    ...
    // Phase 2.5 (2026-05-10): smooth engagement re-emergence. mig 110
    // removed the article_engaged 30-day hard cliff; instead the SQL RPC
    // returns last_engaged_at and we apply smooth attenuation here.
    // 1 / (1 + 0.04 × days_since_engagement). At 30 days = 0.45×;
    // at 90 days = 0.22×; at 365 days = 0.06×. Articles never fully
    // re-emerge but high-quality content can outrank a fresh mediocre
    // alternative — closer to TikTok's suppression curve.
    let engageReEmergeMult = 1.0
    if (a.last_engaged_at) {
      const ts = new Date(a.last_engaged_at).getTime()
      if (Number.isFinite(ts)) {
        const daysSince = Math.max(0, (now - ts) / 86400000)
        engageReEmergeMult = 1.0 / (1.0 + 0.04 * daysSince)
      }
    }
    ...
    a._score = qualityNorm * recency * bucketBoost * stale * eventDemote * catMult * pubMult * seenMult * hourMult * freshMult * negMult * dwellMult * todayPenalty * engageReEmergeMult
  }
...
// Phoenix Phase 7.B (2026-05-09). loadGlobalImpressionBoosts.
//
...
// its "test audience" stage even when ai_final_score is mid-range.
//
// Returns Map<article_id, multiplier ∈ [1.0, 1.5]>. Articles with low
// global impressions get up to 1.5× boost; saturated articles 1.0×.
// Formula: 1 + 0.5 × max(0, 1 - imps / 50).
async function loadGlobalImpressionBoosts(supabase, exploreCandidates) {
  const out = new Map()
  if (!exploreCandidates || exploreCandidates.length === 0) return out
  try {
    const articleIds = exploreCandidates.map(a => a.id).filter(Boolean)
    if (articleIds.length === 0) return out
    const { data, error } = await supabase.rpc('article_global_impression_counts', {
      p_article_ids: articleIds,
    })
    if (error) {
      console.error('[trinity] article_global_impression_counts RPC failed:', error.message)
      return out
    }
    const impsById = new Map()
    for (const row of (data || [])) impsById.set(Number(row.article_id), Number(row.global_impressions) || 0)
    for (const a of exploreCandidates) {
      const imps = impsById.get(Number(a.id)) || 0
      const boost = 1.0 + 0.5 * Math.max(0, 1 - imps / 50)
      if (boost > 1.0) out.set(Number(a.id), boost)
    }
    return out
  } catch (err) {
    console.error('[trinity] loadGlobalImpressionBoosts exception:', err.message)
    return out
  }
}
...
// Phase 1.3 (2026-05-10). loadUserMedianDwellByPrimary.
//
// Returns Map<vq_primary, median_dwell_seconds> for the user's last 30 days,
// minimum 5 events per primary. Feeds dwellMult in rerank() so candidates
// from clusters where the user reads deeply get a boost — replaces mig 097's
// histogram-side watch-time bump.
//
// Cold-start: users with no qualifying events get an empty Map (no boost,
// no demotion). The histogram weights still encode interest direction; this
// is purely the magnitude-of-engagement signal.
async function loadUserMedianDwellByPrimary(supabase, userId) {
  if (!userId) return new Map()
  try {
    const { data, error } = await supabase.rpc('user_median_dwell_by_primary', {
      p_user_id: userId,
      p_days_back: 30,
      p_min_events: 5,
    })
    if (error) {
      console.error('[trinity] user_median_dwell_by_primary RPC failed:', error.message)
      return new Map()
    }
    const out = new Map()
    for (const row of (data || [])) {
      if (row.vq_primary != null && Number.isFinite(Number(row.median_dwell))) {
        out.set(row.vq_primary, Number(row.median_dwell))
      }
    }
    return out
  } catch (err) {
    console.error('[trinity] loadUserMedianDwellByPrimary exception:', err.message)
    return new Map()
  }
}
