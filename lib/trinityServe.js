...
export function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    const recency = recencyWeightForArticle(a, now)
    // 2026-05-17 cleanup: dropped the _redundancyAdjustedScore read — the
    // pipeline column is always NULL (AI scoring is per-article, no view of
    // siblings, so it can't compute event-cluster redundancy). JS-side
    // cross-story protection is via applyDiversityDiscount story axis.
