...
export function phaseAwareSlotBudgets(qualifyingCount, recentEngagementZ) {
  ...
  if (recentEngagementZ > 0.5) {
    return {
      ...
      'trinity-m-tier2': 4,
      'trinity-lt':      2,
      'explore':         4,    // 20% — engaged user, modest explore
    }
  }
  if (recentEngagementZ < -0.5) {
    return {
      ...
      'trinity-m-tier2': 3,
      'trinity-lt':      2,
      'explore':         7,    // 35% — bored user, push discovery
    }
  }
  return {
    'trinity-fresh':   4,
    ...
    'trinity-m-tier2': 3,
    'trinity-lt':      2,
    'explore':         6,      // 30% — neutral heavy user, default
  }
}
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // Phoenix Phase 6.B (2026-05-09): time-of-day primary boost. Loaded
  // separately because it's small + cacheable per-hour (could be optimized
  // later). Per-primary multipliers in [1.0, 1.3] applied during rerank.
  // Phoenix Phase 6.C (2026-05-09): multi-granular negative feedback maps
  // (cluster / source / primary).
  // Phase 1.3 (2026-05-10): per-primary median dwell. Replaces mig 097's
  // histogram-side watch-time bump with a ranker-side multiplier.
  const [hourPrimaryBoosts, negativeDimensions, userMedianDwellByPrimary] = await Promise.all([
  ...
  const rerankOpts = { recentEventIds, categoryMultipliers, publisherPenalties, hourPrimaryBoosts, negativeDimensions, userMedianDwellByPrimary }
  ...
  if (slate.length < feedSize) {
    slate = backfillToTarget(slate, [trendingRanked, ltRanked, mTier2Ranked, freshRanked, mTier1Ranked], feedSize)
  }
...
function rerank(candidates, opts = {}) {
  ...
  for (const a of candidates) {
    ...
    let dwellMult = 1.0
    if (userMedianDwellByPrimary && a.vq_primary != null) {
    ...
    a._score = qualityNorm * recency * bucketBoost * stale * eventDemote * catMult * pubMult * seenMult * hourMult * freshMult * negMult * dwellMult
  }
