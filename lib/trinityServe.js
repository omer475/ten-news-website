...
export async function serveTrinityFeed(supabase, opts) {
  ...
  // Phase 2.2 — tap-then-read funnel stats (replaced dwellMult).
  const [hourPrimaryBoosts, negativeDimensions, userFunnelStats] = await Promise.all([
  ...
  // v5-A. Rerank EACH pool independently (intra-pool by 9-multiplier formula),
  // then compose with fixed slot budgets. Cross-pool ordering is the budget,
  // not a global score sort.
  const rerankOpts = { recentEventIds, categoryMultipliers, hourPrimaryBoosts, negativeDimensions, userFunnelStats, todaysPrimaryCounts }
