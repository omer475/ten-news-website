...
// 2026-05-21 — HARD KILL of the v1 ranker. Production had an environment
// variable RANKER_VERSION=v1 set, which force-enabled the broken model above
// for ALL users. Confirmed root cause of weeks of "old, off-interest" feeds
// (test user 5082a1df): forcing v1 on a clean deployment reproduced an
// 8.5-day-median, World/Politics/Health slate; forcing v0 produced a 24h-
// median Tech slate matching the user's actual taste. The model's
// cluster_fit (topic-match) coefficient is negative and seen_count/age are
// positive, so it literally ranks good-fit fresh articles BELOW stale
// already-seen ones.
//
// Until ranker_v1_model.json is retrained with sign-validated heads, v1 must
// not run regardless of the RANKER_VERSION env var or canary. A plain
// `RANKER_VERSION=v1` is now IGNORED. To deliberately test the model after
// retraining, set RANKER_VERSION=v1-allow-broken (explicit, self-documenting).
const V1_HARD_DISABLED = true
const V1_TEST_OPT_IN = 'v1-allow-broken'

export function pickRankerVersion(userId) {
  const env = process.env.RANKER_VERSION
  // Explicit, deliberate opt-in to the (known-broken) v1 model for retraining
  // validation only. A bare RANKER_VERSION=v1 is intentionally NOT honored.
  if (env === V1_TEST_OPT_IN) return 'v1'
  if (env === 'v0') return 'v0'                  // explicit force-off still works
  if (V1_HARD_DISABLED) return 'v0'              // kill switch — see block comment
  if (env === 'v1') return 'v1'                  // (unreachable while hard-disabled)
...
// 2026-05-20 (PR10) — Trinity-FRESH pool gets a tight publish-time window.
// Personal pool's FRESH_WINDOW_H stays at 720h (30d) for ANN supply, but the
// fresh pool's job is to inject genuinely-fresh content (X tweet-mixer 24h,
// X cr-mixer 720h ceiling). 48h matches X's news-feed default in their open
// source repo. Env override TRINITY_FRESH_POOL_WINDOW_H for live tuning.
const TRINITY_FRESH_POOL_WINDOW_H = Math.max(
  12,
  Number(process.env.TRINITY_FRESH_POOL_WINDOW_H) || 48
)
...
// Category-diversity cap. The cold/trending pools rank by ai_final_score, and
// World/Politics articles score highest on big-news days, so an uncapped slate
// runs ~68% World+Politics (measured 2026-06-13) — "only politics". This
// front-loads each category up to `maxPerCat` from the ranked pool and defers
// the overflow; downstream pick/MMR then draws a diverse top-N while overflow
// still backfills if a thin day lacks other categories (never drops supply).
// World and Politics share one budget (both are "hard global news").
export function capByCategory(ranked, maxPerCat) {
  const norm = (c) => {
    const k = String(c || 'Other').toLowerCase()
    if (k === 'world' || k === 'politics' || k === 'policy') return 'world+politics'
    return k
  }
  const counts = {}
  const kept = []
...
export function phaseAwareSlotBudgets(qualifyingCount, _recentEngagementZ) {
  if (qualifyingCount < EXPLORE_PHASE_THRESHOLDS_QC[0]) {
    // New user (QC < 100): no real taste signal yet, so personalization is
    // mostly noise and the personal pool is empty/backfilled with day-old
    // cosine matches. Lead with FRESH — it's the only recency-guaranteed pool
    // (≤48h, reranked score×recency). This is what made the logged-in feed
    // read "a day old": only 4/20 fresh slots while LT/explore/backfill pulled
    // 24-72h content (measured median 40-45h). Fresh-dominant for new users
    // auto-reverts to SLOT_BUDGETS once they cross QC 100, so engaged users'
    // personalization is untouched. Env knob TRINITY_NEWUSER_FRESH for tuning.
    // fresh 9 / personal 4 / lt 2 / explore 5 = 20, explore held at the 25%
    // cold-user floor (Vombatkere 2024 anti-narrow-trapping invariant). Fresh
    // jumps 4→9 so nearly half the new-user slate is recency-guaranteed.
    return {
      'trinity-fresh':    9,
      'trinity-personal': 4,
      'trinity-lt':       2,
...
export async function serveTrinityFeed(supabase, opts) {
  ...
  const {
    ...
    //
    // 2026-06-01 (precompute cache): when true, skip the exposure/bandit
    // writes (cluster b-score, explore shows, session exposure). The offline
    // precompute cron builds slates the user has NOT seen yet — recording
    // exposure here would poison the fatigue/learning counters. The writes
    // are instead deferred to serve time via recordSlateExposure(), called
    // from the /api/feed/main cache-hit path against the slate actually sent.
    skipExposureWrites = false,
  } = opts
  ...
  if (qualifyingCount < COLD_START_QUALIFYING_FLOOR && !synthDebug.applied) {
    ...
    // 2026-06-13: window 7d → 48h. Cold-start users (everyone at launch, no
    // engagement history) were getting a stale slate (measured median ~40h)
    // from the 7-day trending pool. 48h keeps it news-fresh; retrieveTrending
    // still returns up to TRENDING_POOL_SIZE so there's ample pool for the
    // category cap + MMR below. Env knob TRINITY_COLD_WINDOW_H.
    const cold = await retrieveTrending(supabase, {
      hoursWindow: Number(process.env.TRINITY_COLD_WINDOW_H) || 48,
      excludeIds: seenIds, limit: TRENDING_POOL_SIZE,
    })
    for (const a of cold) a._retriever = 'cold-trending'
    const ranked = rerank(cold)
    // Category cap based on a FIXED 20-card window (not the request limit, which
    // can be 50 → too loose) so the TOP of the feed the user actually sees is
    // diverse. ceil(0.35 × 20) = 7 max per category group; overflow defers to
    // the tail, never dropped.
    const catCap = Math.ceil(0.35 * Math.min(feedSize, 20))
    const diversified = capByCategory(ranked, catCap)
    let picked = mmrDiversify(diversified, feedSize)
    // P1+P3 fix: same X-style diversity discount on cold-start slate.
    picked = applyDiversityDiscount(picked)
    // Final front-load: MMR can re-cluster categories, so re-apply the cap to
    // the picked order — guarantees ≤7 World+Politics in any leading 20.
    picked = capByCategory(picked, catCap)
    return {
      articles: picked,
  ...
  let [personalPool, ltResult, exploreResult, freshPool] = await Promise.all([
    ...
    personalPrimaries.length && userId
      ? retrievePersonalPerPrimary(supabase, userId, personalPrimaries, {
          // 2026-05-21 — personal pool uses a 5-day (120h) window, NOT the 30d
          // FRESH_WINDOW_H. The 30d window flooded the per-primary ANN with old
          // articles (~9:1 old:fresh in the user's top primaries); since the
          // ANN ranks by cosine similarity (age-blind), the nearest matches
          // were mostly 7-24 days old and padded the lower slots. 120h keeps
          // the pool recent while leaving ample supply. Env override:
          // TRINITY_PERSONAL_WINDOW_H. See PERSONAL_POOL_WINDOW_H in trinity.js.
          hoursWindow: PERSONAL_POOL_WINDOW_H,
          minScore: 200,
    ...
    ltClusters.length
      ? retrieveCandidatesAdaptive(supabase, ltClusters, {
          // 2026-05-21 — added minScore: 500 (was unset → 0). The long-tail
          // pool had no quality floor while every other pool did (fresh 300,
          // personal 200, explore 500), so it leaked low-quality filler — e.g.
          // a score-250 "ground ivy garden invader" article reached slot 5 of
          // a live slate. 500 matches the explore pool: both are "reach beyond
          // the user's core interests" channels and should clear a high bar to
          // justify the slot. Keeps the top 70% of articles; LT supply is safe
          // (it widens its time window when thin).
          perClusterLimit: PER_LT_LIMIT, excludeIds: seenIds, minScore: 500,
          minPoolSize: ADAPTIVE_MIN_POOL, userId,
        })
    ...
    freshPrimaries.length
      ? retrievePersonalizedFresh(supabase, freshPrimaries, {
          // 2026-05-20 (PR10) — explicit 48h window on the FRESH pool.
          // PR #197 raised FRESH_WINDOW_H to 720h (30d) for the personal-pool
          // ANN retriever to ensure adequate supply for heavy users. But
          // retrievePersonalizedFresh defaults to the SAME constant, so the
          // "fresh" pool was returning 11-18 day old articles sorted by
          // ai_final_score — exactly the opposite of its purpose. Production
          // audit (test user 5082a1df, slate at 2026-05-19 23:50 UTC) showed
          // trinity-fresh slots filled by 8-18 day old articles while 30+
          // fresh unseen articles in user's top primaries (published in the
          // last 10h) went unserved.
          //
          // Personal pool keeps the 30d window for ANN supply; this pool now
          // returns ONLY content published in the last 48h — peer-aligned
          // with X tweet-mixer's 24h default and Twitter's news-feed pattern.
          hoursWindow: TRINITY_FRESH_POOL_WINDOW_H,
          excludeIds: seenIds,
          limit: FRESH_POOL_LIMIT * 2,
          userId,
          minScore: 300,
        })
      : Promise.resolve([]),
  ...
  // Canonical pick: keep ONE article per embedding story-cluster
  // (_storyClusterId, stamped by assignStoryClusters via single-linkage cosine on
  // embedding_minilm_vec). This is the real cross-pool dedup that the per-pool
  // dedupNearDuplicates AND the ~29%-coverage event tag both miss — verified on
  // the live feed: adjacent near-identical stories had DIFFERENT event tags and
  // DIFFERENT vq, but cluster together on embeddings. `composed` is already
  // ranked, so the first survivor per cluster is the best-for-user representative.
  const collapseByStoryCluster = (items) => {
    const seen = new Set();
    const out = [];
    for (const a of items) {
      const cid = a._storyClusterId;
      if (cid != null) { if (seen.has(cid)) continue; seen.add(cid); }
      out.push(a);
    }
    return out;
  };
  ...
  // Embedding canonical-pick: collapse cross-pool same-event dupes the tag misses.
  slate = collapseByStoryCluster(slate)
  ...
  // Counts kept for debug/observability regardless of whether we write.
  ...
  const exploreShown = slate.filter(a => (a._retrieverTier || a._retriever) === 'explore')
                            .map(a => a.vq_secondary).filter(c => c != null)
  // 2026-06-01: exposure/bandit writes extracted into recordSlateExposure so
  // the precompute-cache serve path can replay them at SERVE time against the
  // slate actually sent. skipExposureWrites suppresses them here for offline
  // precompute (the user hasn't seen the slate yet).
  if (!skipExposureWrites) {
    await recordSlateExposure(supabase, userId, slate)
  }
...
// recordSlateExposure — write the exposure/bandit signals for a slate the user
// is actually being shown. Extracted from serveTrinityFeed (2026-06-01) so the
// precompute-cache path in /api/feed/main can replay these at serve time for a
// cache HIT, while the offline precompute cron skips them (skipExposureWrites).
//
// `slate` items only need: _retriever / _retrieverTier, vq_secondary,
// vq_primary, source — so the cache can persist a lightweight metadata array
// instead of the full article objects. Idempotency note: each call bumps the
// EMAs/exposure once per served slate, exactly as the live path did before.
export async function recordSlateExposure(supabase, userId, slate) {
  if (!Array.isArray(slate) || slate.length === 0) return
  const personalized = new Set(['trinity-personal', 'trinity-lt', 'trinity-fresh'])
  const shownClusterIds = Array.from(new Set(
    slate.filter(a => personalized.has(a._retriever) || personalized.has(a._retrieverTier))
         .map(a => a.vq_secondary).filter(c => c != null)
