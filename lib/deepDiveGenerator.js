...
// Step 1: from recent news headlines, pick the one that opens onto the most
// fascinating story, and name the BROADER timeless topic to actually write
// about (not the news event itself). Returns { index, topic, angle }.
export async function pickInterestingTopic(candidates, avoidTopics = []) {
  const list = candidates
    .map((c, i) => `${i}. [${c.category || '?'}] ${stripMd(c.title)}${c.bullet ? ` — ${stripMd(c.bullet)}` : ''}`)
    .join('\n');
  const avoid = (avoidTopics || []).filter(Boolean).slice(0, 30);
  const prompt = `From the recent news headlines below, choose the ONE that opens
onto the most genuinely fascinating, curiosity-driven story — the kind people
read for the sheer joy of learning ("wait, really?"). Strongly favour science,
space, nature, animals, the ocean, geography, geology, archaeology and history,
the human body, engineering marvels, and surprising numbers or scale. AVOID
routine politics, war/conflict, crime, courts, elections, sports results,
market moves, and celebrity — UNLESS there is a genuinely fascinating timeless
topic underneath them.

Then name the BROADER subject to research and tell as a story — NOT the news
event itself, but the fascinating, evergreen thing behind it. (E.g. an Iceland
eruption → "why Iceland is splitting between two tectonic plates"; an aurora
sighting → "how solar storms make the sky glow".)
${avoid.length ? `\nAvoid subjects close to these recently covered: ${avoid.join('; ')}\n` : ''}
HEADLINES:
${list}

Return ONLY JSON: {"index": number, "topic": "the broader fascinating subject", "angle": "one line on the story to tell"}`;
  const out = await callGemini(prompt);
  return {
    index: Number.isInteger(out.index) ? out.index : 0,
    topic: String(out.topic || '').trim(),
    angle: String(out.angle || '').trim(),
  };
}

function buildInterestingPrompt({ avoidTopics = [], topic = null, seedTitle = null } = {}) {
  const avoid = (avoidTopics || []).filter(Boolean).slice(0, 30);
  const avoidBlock = avoid.length
    ? `\n\nDo NOT pick anything close to these recently-covered topics:\n- ${avoid.join('\n- ')}`
    : '';
  const focus = topic
    ? `Write a fascinating story about THIS subject: ${topic}.${seedTitle ? `\n(A recent news item sparked it: "${stripMd(seedTitle)}" — but write the timeless, fascinating story behind it, not the news event.)` : ''}`
...
people read purely for the joy of learning something.
...
// Pass { topic, seedTitle } to anchor it on a subject chosen from recent news;
// omit them to let the model pick a topic freely.
export async function generateInterestingDeepDive({ avoidTopics = [], topic = null, seedTitle = null } = {}) {
  const { text, sources } = await callGeminiGrounded(buildInterestingPrompt({ avoidTopics, topic, seedTitle }));
