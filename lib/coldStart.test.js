...
function testDecayFactor() {
  ...
  assert(syntheticDecayFactor(40) === 0.5, 'decay: QC=40 → factor=0.5 (Phase 2.4 threshold=80)')
  assert(syntheticDecayFactor(80) === 0.0, 'decay: QC=80 → factor=0.0 (Phase 2.4 threshold)')
...
async function testSynthesizeAtMid() {
  ...
  h1[39] = 100
  const debug = await synthesizeWarmStartHistogram(stub, 'user-1', h1, h2, 40)
  assert(debug.applied === true, 'synthesize-mid: applied=true at QC=40')
  assert(debug.factor === 0.5, 'synthesize-mid: factor=0.5 at QC=40 (Phase 2.4 threshold=80)', `got ${debug.factor}`)
