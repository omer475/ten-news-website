...
function simulateReaction(persona, article, profile, ctx) {
  ...
  if (score >= 0.6) {
    ...
    const shouldLike = Math.random() < 0.35; // 35% like rate for deep reads
    ...
    return { action: 'DEEP_READ', dwell, events, signal: 'ENGAGE', save: shouldSave, liked: shouldLike, matchedSubtopics, moodDelta, score };
  }
  if (score >= 0.3) {
    const dwell = (4 + Math.random() * 10) * patience;
    const shouldSave = Math.random() < persona.personality.saveRate * 0.5;
    const shouldLike = Math.random() < 0.15; // 15% like rate for regular engagement
    const events = ['article_detail_view', 'article_engaged'];
    if (shouldSave) events.push('article_saved');
    if (shouldLike) events.push('article_liked');
    const moodDelta = +(2 + Math.random() * 3);
    return { action: 'ENGAGE', dwell, events, signal: 'ENGAGE', save: shouldSave, liked: shouldLike, matchedSubtopics, moodDelta, score };
  }
  if (score >= 0.05) {
...
async function simulateSession(persona, userId, accessToken, sessionNum, profile, priorHistory = {}) {
  ...
  for (let page = 1; page <= maxPages; page++) {
    ...
    for (const article of freshArticles) {
      ...
      const articleBucket = article.bucket || 'personal';
      for (const eventType of reaction.events) {
        const eventData = {
          event_type: eventType, article_id: parseInt(id), session_id: sessionId,
          category: article.category || null, source: article.source || null, metadata: { bucket: articleBucket },
        };
        if (eventType === 'article_engaged') Object.assign(eventData.metadata, { dwell: String(reaction.dwell.toFixed(1)), engaged_seconds: Math.round(reaction.dwell), total_active_seconds: Math.round(reaction.dwell) });
        else if (eventType === 'article_exit') Object.assign(eventData.metadata, { total_active_seconds: Math.round(reaction.dwell) });
        else if (eventType === 'article_detail_view') Object.assign(eventData.metadata, { dwell: String(reaction.dwell.toFixed(1)) });
        else if (eventType === 'article_liked') Object.assign(eventData.metadata, { liked: true });
      ...
      if (reaction.signal === 'ENGAGE') {
        ...
        if (reaction.liked) ctx.likedCount = (ctx.likedCount || 0) + 1;
        for (const st of reaction.matchedSubtopics) {
      ...
      interactions.push({
        ...
        dwell: reaction.dwell, save: reaction.save, liked: reaction.liked || false,
  ...
  return {
    ...
    stats: {
      total: interactions.length, engaged: ctx.engagedCount, saved: ctx.savedCount, liked: ctx.likedCount || 0,
...
function printSessionDetail(persona, session) {
  ...
  for (const m of session.interactions) {
    ...
    const likeTag = m.liked ? ' ❤️' : '';
    const subtopicStr = m.matchedSubtopics.length > 0 ? m.matchedSubtopics.slice(0, 2).join(', ').substring(0, 32) : '—';

    console.log(`  │  ${String(m.num).padStart(3)}  ${(actionIcon + ' ' + m.action).padEnd(13)} ${dwellStr.padStart(7)}  ${bar.padEnd(20)}  ${String(m.mood).padStart(4)}  ${(m.category || '-').padEnd(14)} ${(m.bucket || '-').padEnd(8)} ${subtopicStr.padEnd(33)} ${(m.title || '').substring(0, 40)}${saveTag}${likeTag}`);
  }
...
function printGrandReport(allResults) {
  ...
  for (const r of ranked) {
    ...
    // Addiction score: combines "improving" AND "maintaining high" signals
    // A user who stays at 95% engagement across 4 sessions IS hooked — they don't need to improve.
    // TikTok/Instagram measure retention (staying high), not just growth.
    let addictionScore = 0;

    // Positive signals: improving OR maintaining at a high level
    const avgEng = engRates.reduce((a, b) => a + b, 0) / engRates.length;
    const avgDwell = dwells.reduce((a, b) => a + b, 0) / dwells.length;
    const avgSkip = skipRates.reduce((a, b) => a + b, 0) / skipRates.length;
    const avgSat = sats.reduce((a, b) => a + b, 0) / sats.length;
    const minEng = Math.min(...engRates);
    const minSat = Math.min(...sats);

    // Engagement: improving OR consistently >= 90%
    if (engT.direction === 'improving') addictionScore++;
    else if (avgEng >= 0.90 && minEng >= 0.80) addictionScore++;  // sustained high

    // Dwell: improving OR consistently >= 8s (deep reading)
    if (dwellT.direction === 'improving') addictionScore++;
    else if (avgDwell >= 8.0) addictionScore++;  // sustained deep engagement

    // Skip rate: declining OR consistently <= 10%
    if (skipT.direction === 'declining') addictionScore++;
    else if (avgSkip <= 0.10) addictionScore++;  // sustained low skip

    // Satisfaction: improving OR consistently >= 80
    if (satT.direction === 'improving') addictionScore++;
    else if (avgSat >= 80 && minSat >= 60) addictionScore++;  // sustained high satisfaction

    // Session length: improving
    if (lenT.direction === 'improving') addictionScore++;

    // Negative signals: only when SIGNIFICANTLY declining
    if (engT.direction === 'declining' && engT.pctChange < -15) addictionScore--;
    if (satT.direction === 'declining' && satT.pctChange < -15) addictionScore--;
    if (skipT.direction === 'improving' && skipT.pctChange > 50) addictionScore--;
  ...
  // Include "maintaining high" as positive — a feed that stays at 93% IS learning/personalized
  const avgOverallEng = allInts.length > 0 ? allInts.filter(i => i.signal === 'ENGAGE' || i.signal === 'DEEP_READ').length / allInts.length : 0;
  const feedLearns = engTrend.direction === 'improving' || relevTrend.direction === 'improving' || avgOverallEng >= 0.90;
  const usersGetHooked = totalAddicted > totalDisengaging;
  const satisfactionGrows = satTrend.direction === 'improving' || satTrend.direction === 'flat';
