...
// ============================================================================
// 20-PERSONA FEED + EXPLORE TEST V4
// FIXES from V3:
// - Uses SHORT topic IDs like the real iOS app sends (e.g., "ai", "sports")
// - All 20 brand-new diverse personas (different from V3)
...
const PASSWORD = 'TestPersonaV4_2024!';
...
// ============================================================================
// 20 PERSONAS — all fresh (zero history), using SHORT topic IDs
// ============================================================================
...
async function setupPersona(persona) {
  ...
  if (existing) {
    ...
    const { data, error } = await adminDb.auth.admin.createUser({
      ...
      user_metadata: { full_name: `${persona.name} (Test V4)` },
    });
  ...
  const profileData = {
    email: persona.email,
    full_name: `${persona.name} (Test V4)`,
...
async function readFeedWithExploreChecks(persona, userId, accessToken, profile) {
  const sessionId = `feed_v4_${persona.name.toLowerCase()}_${Date.now()}`;
...
async function main() {
  const T = '='.repeat(120);
  console.log(T);
  console.log('  20-PERSONA FEED + EXPLORE TEST V4');
  console.log('  - All 20 personas start from ZERO (full wipe)');
  ...
  for (let i = 0; i < PERSONAS.length; i++) {
    ...
    console.log(`\n${'─'.repeat(120)}`);
    console.log(`  [${i + 1}/20] ${persona.name.toUpperCase()}, ${persona.age} — ${persona.location}`);
  ...
  console.log(`\n\n${'='.repeat(140)}`);
  console.log('  GRAND REPORT — 20-PERSONA FEED + EXPLORE TEST V4');
