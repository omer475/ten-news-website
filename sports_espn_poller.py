...
TRACKING_TABLE_AVAILABLE = False

def ensure_tracking_table(supabase):
    """Check if sports_poll_tracking table exists. Fall back to local JSON tracking."""
    global TRACKING_TABLE_AVAILABLE
    try:
        supabase.table('sports_poll_tracking').select('id').limit(1).execute()
        TRACKING_TABLE_AVAILABLE = True
        print("   ✅ sports_poll_tracking table found")
    except Exception:
        TRACKING_TABLE_AVAILABLE = False
        print("   ⚠️ sports_poll_tracking table not found — using local JSON tracking")
        print("   📋 Run migrations/016_sports_poll_tracking.sql in Supabase SQL Editor to create it")


# Local file-based tracking fallback
LOCAL_TRACKING_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.sports_poll_tracking.json')

def _load_local_tracking():
    try:
...
def is_event_already_processed(supabase, event_id: str) -> bool:
    """Check if event was already processed (DB table or local JSON fallback)."""
    if TRACKING_TABLE_AVAILABLE:
        try:
    ...
    tracking = _load_local_tracking()
    return event_id in tracking
...
def publish_sports_article(supabase, league_config: Dict, event: Dict,
    ...
    # Create a cluster row first (published_articles has FK to clusters)
    try:
        cluster_data = {
            'main_title': generated['title'],
            'status': 'active',
            'source_count': 1,
...
def track_processed_event(supabase, event: Dict, published_id: Optional[int]):
    """Record processed event in tracking table + local JSON fallback."""
    if TRACKING_TABLE_AVAILABLE:
        try:
    ...
    # Always update local tracking too
    tracking = _load_local_tracking()
    tracking[event['event_id']] = {
        'sport': event['sport'],
        'event_name': event['event_name'],
        'published_article_id': published_id,
        'processed_at': datetime.now().isoformat(),
    }
    _save_local_tracking(tracking)
