...
// ============================================================================
// 50-PERSONA FEED TEST V7 — 500+ articles per persona, ultra-realistic behavior
// Key upgrades over V6:
//   - 10 sessions/persona, dynamic session lengths (mood-driven)
//   - History carries over between sessions (feed can learn)
//   - Flow state / frustration cascade / interest spikes
//   - 500+ articles per persona (~25,000 total reads)
//   - Deep analytics: learning rate, aha moments, churn risk, gen comparison
// ============================================================================
...
const TARGET_ARTICLES_PER_PERSONA = 500;
...
async function simulateSession(persona, userId, accessToken, sessionNum, profile, priorHistory = {}) {
  const sessionId = `sim7_${persona.name.toLowerCase()}_s${sessionNum}_${Date.now()}`;

  // Dynamic session size: base range from personality + mood-based extension
  const [baseMin, baseMax] = persona.personality.baseSessions;
  const baseTarget = baseMin + Math.floor(Math.random() * (baseMax - baseMin));
  // Later sessions: returning users browse more if they've been satisfied before
  const returnBoost = sessionNum > 1 ? Math.min(10, sessionNum * 1.5) : 0;
  const maxArticlesBase = baseTarget + Math.round(returnBoost);
  const maxPages = Math.ceil(maxArticlesBase * 1.5 / 25) + 2; // extra pages for flow extension
...
function printGrandReport(allResults) {
  ...
  console.log(`\n\n${THICK}`);
  console.log('  GRAND REPORT — 50 PERSONAS V7 (500+ articles/persona, accumulated history, flow state)');
  console.log(THICK);
...
async function main() {
  ...
  fs.writeFileSync('test_50persona_results_v7.json', JSON.stringify(jsonResults, null, 2));
  console.log('\nResults saved to test_50persona_results_v7.json');
}
