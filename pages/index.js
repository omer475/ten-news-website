...
export default function Home() {
  ...
  // Helper: Get adaptive neutral tone based on hue
  const getAdaptiveNeutralTone = (hue) => {
    // Blue hue range: 200-260 degrees
    if (hue >= 200 && hue <= 260) {
      return [26, 39, 57]; // #1A2739 dark navy
    }
    // Green hue range: 100-160 degrees
    if (hue >= 100 && hue <= 160) {
      return [30, 56, 42]; // #1E382A forest green
    }
    // Orange/Red hue range: 0-50 or 320-360
    if ((hue >= 0 && hue <= 50) || (hue >= 320 && hue <= 360)) {
      return [59, 36, 26]; // #3B241A deep brown
    }
    // Gray/neutral (low saturation or mid-range hues)
    return [43, 43, 43]; // #2B2B2B graphite gray
  };

  // Function to calculate adaptive highlight color based on new rules
  const getAdaptiveHighlightColor = (blurColor) => {
    ...
    // Convert to HSL to get hue and lightness
    const [h, s, l] = rgbToHsl(r, g, b);
    
    // Background is dark if lightness < 0.5
    const isDarkBackground = l < 0.5;
    
    // Calculate highlight color based on background
    let highlightR, highlightG, highlightB;
    
    if (isDarkBackground) {
      // Brighten and saturate: +20% sat, +30% brightness
      const newS = Math.min(1, s + 0.2);
      const newL = Math.min(1, l + 0.3);
      [highlightR, highlightG, highlightB] = hslToRgb(h, newS, newL);
    } else {
      // Darken and desaturate: -20% brightness, -15% sat
      const newS = Math.max(0, s - 0.15);
      const newL = Math.max(0, l - 0.2);
      [highlightR, highlightG, highlightB] = hslToRgb(h, newS, newL);
    }
    
    // Check contrast ratio with blurred background (approximate background as darkened version)
    const bgR = Math.max(0, r - 50);
    const bgG = Math.max(0, g - 50);
    const bgB = Math.max(0, b - 50);
    const contrastRatio = getContrastRatio([highlightR, highlightG, highlightB], [bgR, bgG, bgB]);
    
    // Fallback if contrast too low or color too close to white
    const isTooLight = highlightR > 240 && highlightG > 240 && highlightB > 240;
    const isTooDark = highlightR < 15 && highlightG < 15 && highlightB < 15;
    
    if (contrastRatio < 4.5 || isTooLight || isTooDark) {
      // Use adaptive neutral tone based on hue
      [highlightR, highlightG, highlightB] = getAdaptiveNeutralTone(h);
    }
    
    return `rgb(${Math.round(highlightR)}, ${Math.round(highlightG)}, ${Math.round(highlightB)})`;
  };

  // Function to get default text color based on background lightness
  const getDefaultTextColor = (blurColor) => {
    if (!blurColor) return '#FFFFFF';
    ...
    return l < 0.5 ? '#FFFFFF' : '#1E1E1E';
  };

  // Function to extract dominant color from blurred region (bottom 45% of image)
  const extractDominantColor = (imgElement, storyIndex) => {
    try {
      ...
      // Sample from bottom 45% of image (blurred region)
      const blurRegionStart = Math.floor(canvas.height * 0.55);
      const imageData = ctx.getImageData(0, blurRegionStart, canvas.width, canvas.height - blurRegionStart);
      const pixels = imageData.data;
      
      // Sample pixels from blurred region
      const colorMap = {};
      ...
      // Find most common color from blurred region
      let maxCount = 0;
      ...
      if (dominantColor) {
        ...
        // Store the color for use in highlights
        setImageDominantColors(prev => ({ 
          ...prev, 
          [storyIndex]: { 
            original: rgbaColor, 
            light: rgbaColor, 
            blurRegion: rgbaColor,
        ...
        setImageDominantColors(prev => ({ 
          ...prev, 
          [storyIndex]: { 
            original: 'rgba(0, 0, 0, 1.0)', 
            light: 'rgba(0, 0, 0, 1.0)',
            blurRegion: 'rgba(0, 0, 0, 1.0)',
            lightness: 0
          }
        }));
      }
  ...
  const renderBoldText = (text, blurColor) => {
    ...
    // Use blur region color from stored data, fallback to provided blurColor
    const colorToUse = blurColor || '#FFFFFF';
    const highlightColor = getAdaptiveHighlightColor(colorToUse) || '#FFFFFF';
  ...
  const renderTitleWithHighlight = (text, blurColor) => {
    ...
    // Use blur region color from stored data, fallback to provided blurColor
    const colorToUse = blurColor || '#000000';
    const highlightColor = getAdaptiveHighlightColor(colorToUse);
                    ...
                    <div style={{
                      ...
                      <div style={{
                        ...
                        <h3 style={{ 
                          ...
                          lineHeight: '1.2',
                          letterSpacing: '-0.5px',
                          color: getDefaultTextColor(imageDominantColors[index]?.blurRegion || imageDominantColors[index]?.original),
                          textShadow: imageDominantColors[index]?.lightness < 0.5 
                            ? '0 2px 8px rgba(0,0,0,0.4), 0 4px 16px rgba(0,0,0,0.2)'
                            : '0 2px 8px rgba(255,255,255,0.3), 0 4px 16px rgba(255,255,255,0.1)'
                        }}>{renderTitleWithHighlight(story.title, imageDominantColors[index]?.blurRegion || imageDominantColors[index]?.original)}</h3>
                      </div>
                    </div>
                              ...
                              <ul style={{
                                ...
                                {story.summary_bullets.map((bullet, i) => (
                                  <li key={i} style={{
                                    ...
                                    {renderBoldText(bullet, imageDominantColors[index]?.blurRegion || imageDominantColors[index]?.original)}
                                  </li>
                                ))}
                              </ul>
                            ) : (
