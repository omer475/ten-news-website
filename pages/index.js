...
import { EmailConfirmation, ResetPasswordModal, AuthPanel } from '../components/AuthForms';
...
import { hslToRgb, toHex } from '../utils/colorUtils';
import {
  extractColorfulCandidates, selectColorForArticle,
  createBlurColor, createTitleHighlightColor,
  createBulletTextColor, createInfoBoxColor
...
export default function Home({ initialNews, initialWorldEvents }) {
  ...
  // NOTE: The old full-screen TikTok-pager navigation effect (document-level
  // non-passive touchstart/touchmove/touchend/wheel/keydown listeners that
  // called preventDefault to hijack scroll) was removed during the pivot to a
  // continuous-scroll feed. It froze native scrolling on iOS Safari.

  // Continuous-scroll feed: ensure the page scrolls natively. Clears any
  // leftover inline body lock so globals.css (overflow-y:auto) applies. The
  // old TikTok-pager scroll lock froze native touch scrolling on iOS Safari.
  useEffect(() => {
    ...
    <>
      ...
      <style jsx global>{`
        * {
        ...
        .auth-modal {
          background: color-mix(in srgb, ${darkMode ? 'rgba(30, 30, 30, 0.8)' : 'rgba(255, 255, 255, 0.75)'} 90%, transparent);
          ...
          pointer-events: auto;
          box-shadow:
            inset 0 0 0 0.5px ${darkMode ? 'rgba(255, 255, 255, 0.08)' : 'rgba(255, 255, 255, 0.5)'},
            inset 0 1px 0 0 ${darkMode ? 'rgba(255, 255, 255, 0.05)' : 'rgba(255, 255, 255, 0.8)'},
            0 8px 40px rgba(0, 0, 0, 0.15),
            0 2px 12px rgba(0, 0, 0, 0.08);
          border: 1px solid ${darkMode ? 'rgba(255, 255, 255, 0.06)' : 'rgba(255, 255, 255, 0.4)'};
        }
        ...
        .auth-modal-footer {
          margin-top: 20px;
          padding-top: 16px;
          border-top: 1px solid ${darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.06)'};
          text-align: center;
        }
...
export async function getServerSideProps({ req, res }) {
  ...
  // every visitor for the cache window, so the feed looked identical on refresh
  // (the #1 cause of "I always see the same articles"). The feed must be fresh
  // per load — server ranking now jitters per call and the client re-fetches all
  // articles + filters read ones, so each load is genuinely different.
