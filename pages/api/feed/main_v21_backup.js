...
// Per MEGA_RAPOR: TikTok, Pinterest, Instagram all use embeddings as core.
// ═══════════════════════════════════════════════════════════════
...
async function computeSessionVector(supabase, engagedIds, skippedIds) {
  ...
  if (engagedIds.length > 0) {
    const { data: engaged } = await supabase.from('published_articles').select('embedding_minilm').in('id', engagedIds.slice(0, 20));
    if (engaged && engaged.length > 0) {
      const avg = new Array(DIM).fill(0); let count = 0;
      for (const a of engaged) { if (!a.embedding_minilm || !Array.isArray(a.embedding_minilm)) continue; for (let i = 0; i < DIM; i++) avg[i] += a.embedding_minilm[i]; count++; }
      if (count > 0) { for (let i = 0; i < DIM; i++) avg[i] /= count; sessionVector = avg; }
    }
  }
  if (skippedIds.length > 0 && sessionVector) {
    const { data: skipped } = await supabase.from('published_articles').select('embedding_minilm').in('id', skippedIds.slice(0, 20));
    if (skipped && skipped.length > 0) {
      const skipAvg = new Array(DIM).fill(0); let count = 0;
      for (const a of skipped) { if (!a.embedding_minilm || !Array.isArray(a.embedding_minilm)) continue; for (let i = 0; i < DIM; i++) skipAvg[i] += a.embedding_minilm[i]; count++; }
      if (count > 0) { for (let i = 0; i < DIM; i++) sessionVector[i] -= 0.3 * skipAvg[i] / count; }
    }
  }
...
function scoreEmbedding(article, pgvecSimilarity, sessionVector, skipProfile) {
  ...
  if (skipProfile) {
    const tags = safeJsonParse(article.interest_tags, []);
    const n = Math.max(tags.length, 1); let skipScore = 0;
    const now = Date.now(); const HALF = 7 * 24 * 3600000;
    for (const tag of tags) {
      const entry = skipProfile[tag.toLowerCase()];
      if (typeof entry === 'object' && entry?.w) { skipScore += entry.w * Math.exp(-0.693 * (now - new Date(entry.t || 0).getTime()) / HALF); }
      else if (typeof entry === 'number') { skipScore += entry; }
    }
...
function buildEmbeddingCache(articles) {
  const cache = new Map();
  for (const a of articles) { if (a.embedding_minilm && Array.isArray(a.embedding_minilm)) cache.set(a.id, a.embedding_minilm); }
  return cache;
}
...
async function handleV2Feed(req, res, supabase, opts) {
  ...
  if (!hasAnyPersonalization && interestCategories.size > 0 && iPool.length > 0) {
    ...
    // ═══════════════════════════════════════════════════════
    // MULTI-INTEREST INTERLEAVING + TRENDING ZONES
    //
    // Problem: if user likes Galatasaray (0.5), AI (0.3), F1 (0.2),
    // a simple top-N by score fills all slots with Galatasaray.
    // Fix: group articles by their best-matching interest, then
    // round-robin between interest groups so ALL interests appear.
    //
    // Also: every 4th slot = trending (big events everyone cares about)
    // Also: max 2 consecutive from same interest group
    // ═══════════════════════════════════════════════════════
    ...
    const scoredInterest = iPool.map(a => {
      ...
      for (const tag of tags) {
        const t = tag.toLowerCase();
        const ts = effectiveTagProfile[t] || 0;
        totalTagScore += ts;
        if (ts > bestTagScore) { bestTagScore = ts; bestTag = t; }
        if (hasSP) {
    ...
    if (hasTP) {
      const groupSummary = groupKeys.slice(0, 5).map(g => `${g}:${interestGroups[g]?.length || 0}`).join(', ');
      console.log(`[feed] MULTI-INTEREST: ${selected.length} articles, ${groupKeys.length} groups, ${trendingInserted} trending | ${groupSummary} | for ${userId?.substring(0,8)}`);
    }
    ...
    // Same multi-interest interleaving as cold-start mode above
    ...
    const scoredInterest2 = allInterest.map(a => {
      ...
      totalTagScore /= n; skipPen /= n;
      const recency = getRecencyDecay(a.created_at, a.category, a.shelf_life_days);
      const quality = ((a.ai_final_score || 0) / 1000) * recency;
      const score = hasTP2 ? totalTagScore * 500 - skipPen * 300 + quality * 100 : quality * 250;
      const group = bestTag || (a.category || 'Other').toLowerCase();
      return { ...a, _pScore: score, _group: group };
    });

    const iGroups2 = {};
    for (const a of scoredInterest2) {
      if (!iGroups2[a._group]) iGroups2[a._group] = [];
      iGroups2[a._group].push(a);
    }
    for (const g of Object.values(iGroups2)) g.sort((a, b) => b._pScore - a._pScore);
    ...
    while (selected.length < limit) {
      ...
      if (!picked) {
        ...
        if (!picked) picked = mmrSelectDeduped(tPool, selected, tagCache, 0.8);
        if (!picked) break;
        picked.bucket = 'discovery'; recordEventSelection(picked); selected.push(picked); lastG2 = '__d__'; consec2 = 1; continue;
      }
