...
// Re-ranks by importance * 0.5^(ageHours/halfLife) * (1 ± jitter) so the order
// varies on every call — the feed visibly changes on refresh. The API is NOT
// edge-cached (vercel.json no-cache + client cache-buster), so jitter is safe.
// ±18% reshuffles the top tier per load but is bounded so a low-score article
// can't leap to #1. Mirrors utils/sortArticles.applyFreshness.
